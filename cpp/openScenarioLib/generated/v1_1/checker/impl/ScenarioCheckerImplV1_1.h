        ...
        */
        ...
        private:
            ...
            void ApplyAbsoluteSpeedCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IAbsoluteSpeed> object, const ContextMode contextMode)
            {
                ...
                // getChildren
                const auto kSteadyState = object->GetSteadyState();
                if ( kSteadyState)
                {
            ...
            template <class T>
            void ApplyDeterministicCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IDeterministic> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IDeterministic>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kDeterministicParameterDistributions = object->GetDeterministicParameterDistributions();
            ...
            template <class T>
            void ApplyDeterministicMultiParameterDistributionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IDeterministicMultiParameterDistribution> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IDeterministicMultiParameterDistribution>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kDeterministicMultiParameterDistributionType = object->GetDeterministicMultiParameterDistributionType();
            ...
            template <class T>
            void ApplyDeterministicMultiParameterDistributionTypeCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IDeterministicMultiParameterDistributionType> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IDeterministicMultiParameterDistributionType>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kValueSetDistribution = object->GetValueSetDistribution();
            ...
            template <class T>
            void ApplyDeterministicParameterDistributionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IDeterministicParameterDistribution> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IDeterministicParameterDistribution>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kDeterministicMultiParameterDistribution = object->GetDeterministicMultiParameterDistribution();
                ...
                const auto kDeterministicSingleParameterDistribution = object->GetDeterministicSingleParameterDistribution();
            ...
            template <class T>
            void ApplyDeterministicSingleParameterDistributionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IDeterministicSingleParameterDistribution> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IDeterministicSingleParameterDistribution>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kDeterministicSingleParameterDistributionType = object->GetDeterministicSingleParameterDistributionType();
            ...
            template <class T>
            void ApplyDeterministicSingleParameterDistributionTypeCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IDeterministicSingleParameterDistributionType> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IDeterministicSingleParameterDistributionType>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kDistributionSet = object->GetDistributionSet();
                if ( kDistributionSet)
                {
                    ApplyDistributionSetCheckerRules(messageLogger, kDistributionSet, contextMode);
                }
                const auto kDistributionRange = object->GetDistributionRange();
                if ( kDistributionRange)
                {
                    ApplyDistributionRangeCheckerRules(messageLogger, kDistributionRange, contextMode);
                }
                const auto kUserDefinedDistribution = object->GetUserDefinedDistribution();
            ...
            template <class T>
            void ApplyDistributionDefinitionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IDistributionDefinition> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IDistributionDefinition>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kDeterministic = object->GetDeterministic();
                if ( kDeterministic)
                {
                    ApplyDeterministicCheckerRules(messageLogger, kDeterministic, contextMode);
                }
                const auto kStochastic = object->GetStochastic();
                if ( kStochastic)
                {
            ...
            template <class T>
            void ApplyDistributionRangeCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IDistributionRange> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IDistributionRange>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kRange = object->GetRange();
                if ( kRange)
                {
            ...
            template <class T>
            void ApplyDistributionSetCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IDistributionSet> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IDistributionSet>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kElements = object->GetElements();
            ...
            template <class T>
            void ApplyDistributionSetElementCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IDistributionSetElement> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IDistributionSetElement>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            void ApplyEntityObjectCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IEntityObject> object, const ContextMode contextMode)
            {
                ...
                const auto kExternalObjectReference = object->GetExternalObjectReference();
            ...
            template <class T>
            void ApplyExternalObjectReferenceCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IExternalObjectReference> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IExternalObjectReference>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            void ApplyFileHeaderCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IFileHeader> object, const ContextMode contextMode)
            {
                ...
                // getChildren
                const auto kLicense = object->GetLicense();
                if ( kLicense)
                {
            ...
            void ApplyFollowTrajectoryActionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IFollowTrajectoryAction> object, const ContextMode contextMode)
            {
                ...
                const auto kTrajectoryRef = object->GetTrajectoryRef();
                if ( kTrajectoryRef)
                {
            ...
            template <class T>
            void ApplyGeoPositionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IGeoPosition> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IGeoPosition>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            template <class T>
            void ApplyHistogramCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IHistogram> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IHistogram>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kBins = object->GetBins();
            ...
            template <class T>
            void ApplyHistogramBinCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IHistogramBin> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IHistogramBin>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            template <class T>
            void ApplyLicenseCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<ILicense> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<ILicense>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            template <class T>
            void ApplyNormalDistributionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<INormalDistribution> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<INormalDistribution>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            void ApplyOpenScenarioCategoryCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IOpenScenarioCategory> object, const ContextMode contextMode)
            {
                ...
                const auto kParameterValueDistributionDefinition = object->GetParameterValueDistributionDefinition();
            ...
            void ApplyParameterDeclarationCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IParameterDeclaration> object, const ContextMode contextMode)
            {
                ...
                // getChildren
                const auto kConstraintGroups = object->GetConstraintGroups();
            ...
            template <class T>
            void ApplyParameterValueDistributionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IParameterValueDistribution> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IParameterValueDistribution>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kScenarioFile = object->GetScenarioFile();
                if ( kScenarioFile)
                {
                    ApplyFileCheckerRules(messageLogger, kScenarioFile, contextMode);
                }
                const auto kDistributionDefinition = object->GetDistributionDefinition();
            ...
            template <class T>
            void ApplyParameterValueDistributionDefinitionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IParameterValueDistributionDefinition> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IParameterValueDistributionDefinition>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kParameterValueDistribution = object->GetParameterValueDistribution();
            ...
            template <class T>
            void ApplyParameterValueSetCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IParameterValueSet> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IParameterValueSet>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            template <class T>
            void ApplyPoissonDistributionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IPoissonDistribution> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IPoissonDistribution>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            void ApplyPositionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IPosition> object, const ContextMode contextMode)
            {
                ...
                const auto kGeoPosition = object->GetGeoPosition();
                if ( kGeoPosition)
                {
                    ApplyGeoPositionCheckerRules(messageLogger, kGeoPosition, contextMode);
                }
                const auto kTrajectoryPosition = object->GetTrajectoryPosition();
            ...
            template <class T>
            void ApplyProbabilityDistributionSetCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IProbabilityDistributionSet> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IProbabilityDistributionSet>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            template <class T>
            void ApplyProbabilityDistributionSetElementCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IProbabilityDistributionSetElement> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IProbabilityDistributionSetElement>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            template <class T>
            void ApplyRangeCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IRange> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IRange>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            void ApplyRoadNetworkCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IRoadNetwork> object, const ContextMode contextMode)
            {
                ...
                const auto kUsedArea = object->GetUsedArea();
                if ( kUsedArea)
                {
            ...
            template <class T>
            void ApplySteadyStateCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<ISteadyState> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<ISteadyState>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kTargetDistanceSteadyState = object->GetTargetDistanceSteadyState();
                ...
                const auto kTargetTimeSteadyState = object->GetTargetTimeSteadyState();
            ...
            template <class T>
            void ApplyStochasticCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IStochastic> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IStochastic>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kStochasticDistributions = object->GetStochasticDistributions();
            ...
            template <class T>
            void ApplyStochasticDistributionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IStochasticDistribution> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IStochasticDistribution>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kStochasticDistributionType = object->GetStochasticDistributionType();
            ...
            template <class T>
            void ApplyStochasticDistributionTypeCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IStochasticDistributionType> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IStochasticDistributionType>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kProbabilityDistributionSet = object->GetProbabilityDistributionSet();
            ...
            template <class T>
            void ApplyTargetDistanceSteadyStateCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<ITargetDistanceSteadyState> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<ITargetDistanceSteadyState>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            template <class T>
            void ApplyTargetTimeSteadyStateCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<ITargetTimeSteadyState> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<ITargetTimeSteadyState>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            void ApplyTrafficActionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<ITrafficAction> object, const ContextMode contextMode)
            {
                ...
                const auto kTrafficStopAction = object->GetTrafficStopAction();
            ...
            template <class T>
            void ApplyTrafficStopActionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<ITrafficStopAction> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<ITrafficStopAction>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            template <class T>
            void ApplyTrajectoryPositionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<ITrajectoryPosition> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<ITrajectoryPosition>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            template <class T>
            void ApplyTrajectoryRefCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<ITrajectoryRef> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<ITrajectoryRef>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            template <class T>
            void ApplyUniformDistributionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IUniformDistribution> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IUniformDistribution>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            template <class T>
            void ApplyUsedAreaCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IUsedArea> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IUsedArea>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kPositions = object->GetPositions();
            ...
            template <class T>
            void ApplyUserDefinedDistributionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IUserDefinedDistribution> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IUserDefinedDistribution>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            template <class T>
            void ApplyValueConstraintCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IValueConstraint> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IValueConstraint>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
            ...
            template <class T>
            void ApplyValueConstraintGroupCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IValueConstraintGroup> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IValueConstraintGroup>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kConstraints = object->GetConstraints();
            ...
            template <class T>
            void ApplyValueSetDistributionCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IValueSetDistribution> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IValueSetDistribution>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
                ...
                // getChildren
                const auto kParameterValueSets = object->GetParameterValueSets();
            ...
            void ApplyWeatherCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IWeather> object, const ContextMode contextMode)
            {
                ...
                const auto kWind = object->GetWind();
                if ( kWind)
                {
            ...
            template <class T>
            void ApplyWindCheckerRules(std::shared_ptr<IContentMessageLogger<T>> messageLogger, const std::shared_ptr<IWind> object, const ContextMode contextMode)
            {
                ...
                if ( !checkerRulesForType.empty() )
                {
                    for (auto& checkerRule:checkerRulesForType)
                    {
                        auto typedCheckerRule = std::dynamic_pointer_cast<ICheckerRule<IWind>> (checkerRule);
                        if (contextMode == MODE_FILE) 
                        {
                            typedCheckerRule->ApplyRuleInFileContext(std::dynamic_pointer_cast<IParserMessageLogger>(messageLogger), object);
                        }
        ...
        public:
            ...
            OPENSCENARIOLIB_EXP void AddDeterministicCheckerRule(std::shared_ptr<ICheckerRule<IDeterministic>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddDeterministicMultiParameterDistributionCheckerRule(std::shared_ptr<ICheckerRule<IDeterministicMultiParameterDistribution>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddDeterministicMultiParameterDistributionTypeCheckerRule(std::shared_ptr<ICheckerRule<IDeterministicMultiParameterDistributionType>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddDeterministicParameterDistributionCheckerRule(std::shared_ptr<ICheckerRule<IDeterministicParameterDistribution>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddDeterministicSingleParameterDistributionCheckerRule(std::shared_ptr<ICheckerRule<IDeterministicSingleParameterDistribution>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddDeterministicSingleParameterDistributionTypeCheckerRule(std::shared_ptr<ICheckerRule<IDeterministicSingleParameterDistributionType>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddDistributionDefinitionCheckerRule(std::shared_ptr<ICheckerRule<IDistributionDefinition>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddDistributionRangeCheckerRule(std::shared_ptr<ICheckerRule<IDistributionRange>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddDistributionSetCheckerRule(std::shared_ptr<ICheckerRule<IDistributionSet>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddDistributionSetElementCheckerRule(std::shared_ptr<ICheckerRule<IDistributionSetElement>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddExternalObjectReferenceCheckerRule(std::shared_ptr<ICheckerRule<IExternalObjectReference>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddGeoPositionCheckerRule(std::shared_ptr<ICheckerRule<IGeoPosition>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddGlobalActionCheckerRule(std::shared_ptr<ICheckerRule<IGlobalAction>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddHistogramCheckerRule(std::shared_ptr<ICheckerRule<IHistogram>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddHistogramBinCheckerRule(std::shared_ptr<ICheckerRule<IHistogramBin>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddLicenseCheckerRule(std::shared_ptr<ICheckerRule<ILicense>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddNormalDistributionCheckerRule(std::shared_ptr<ICheckerRule<INormalDistribution>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddParameterValueDistributionCheckerRule(std::shared_ptr<ICheckerRule<IParameterValueDistribution>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddParameterValueDistributionDefinitionCheckerRule(std::shared_ptr<ICheckerRule<IParameterValueDistributionDefinition>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddParameterValueSetCheckerRule(std::shared_ptr<ICheckerRule<IParameterValueSet>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddPoissonDistributionCheckerRule(std::shared_ptr<ICheckerRule<IPoissonDistribution>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddProbabilityDistributionSetCheckerRule(std::shared_ptr<ICheckerRule<IProbabilityDistributionSet>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddProbabilityDistributionSetElementCheckerRule(std::shared_ptr<ICheckerRule<IProbabilityDistributionSetElement>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddRangeCheckerRule(std::shared_ptr<ICheckerRule<IRange>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddSteadyStateCheckerRule(std::shared_ptr<ICheckerRule<ISteadyState>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddStochasticCheckerRule(std::shared_ptr<ICheckerRule<IStochastic>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddStochasticDistributionCheckerRule(std::shared_ptr<ICheckerRule<IStochasticDistribution>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddStochasticDistributionTypeCheckerRule(std::shared_ptr<ICheckerRule<IStochasticDistributionType>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddTargetDistanceSteadyStateCheckerRule(std::shared_ptr<ICheckerRule<ITargetDistanceSteadyState>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddTargetTimeSteadyStateCheckerRule(std::shared_ptr<ICheckerRule<ITargetTimeSteadyState>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddTrafficStopActionCheckerRule(std::shared_ptr<ICheckerRule<ITrafficStopAction>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddTrajectoryPositionCheckerRule(std::shared_ptr<ICheckerRule<ITrajectoryPosition>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddTrajectoryRefCheckerRule(std::shared_ptr<ICheckerRule<ITrajectoryRef>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddUniformDistributionCheckerRule(std::shared_ptr<ICheckerRule<IUniformDistribution>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddUsedAreaCheckerRule(std::shared_ptr<ICheckerRule<IUsedArea>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddUserDefinedDistributionCheckerRule(std::shared_ptr<ICheckerRule<IUserDefinedDistribution>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddValueConstraintCheckerRule(std::shared_ptr<ICheckerRule<IValueConstraint>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddValueConstraintGroupCheckerRule(std::shared_ptr<ICheckerRule<IValueConstraintGroup>> checkerRule) override;
            OPENSCENARIOLIB_EXP void AddValueSetDistributionCheckerRule(std::shared_ptr<ICheckerRule<IValueSetDistribution>> checkerRule) override;
            ...
            OPENSCENARIOLIB_EXP void AddWindCheckerRule(std::shared_ptr<ICheckerRule<IWind>> checkerRule) override;
