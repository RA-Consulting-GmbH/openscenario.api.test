...
namespace NET_ASAM_OPENSCENARIO
{
    namespace v1_3
    {
        ...
        void OpenScenarioXmlExporter::FillAccelerationConditionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IAccelerationConditionWriter> accelerationConditionWriter)
        {
            ...
            if (!( kDirection == DirectionalDimension::UNKNOWN) || accelerationConditionWriter->IsSetDirection())
            {
        ...
        void OpenScenarioXmlExporter::FillActivateControllerActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IActivateControllerActionWriter> activateControllerActionWriter)
        {
            ...
            if (!( kAnimation == false) || activateControllerActionWriter->IsSetAnimation())
            {
            ...
            if (!( kLighting == false) || activateControllerActionWriter->IsSetLighting())
            {
            ...
            const auto kObjectControllerRef = activateControllerActionWriter->GetObjectControllerRef();
            if (kObjectControllerRef)
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__OBJECT_CONTROLLER_REF.c_str(), XmlExportHelper::ToXmlStringFromString(kObjectControllerRef->GetNameRef()).c_str());
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        ...
        void OpenScenarioXmlExporter::FillAngleConditionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IAngleConditionWriter> angleConditionWriter)
        {
            // Add Attributes (Parameters)
            const auto kAngle = angleConditionWriter->GetAngle();
            if (angleConditionWriter->IsAngleParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ANGLE.c_str(), angleConditionWriter->GetParameterFromAngle().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ANGLE.c_str(), XmlExportHelper::ToXmlStringFromDouble( kAngle).c_str());
            }
            const auto kAngleTolerance = angleConditionWriter->GetAngleTolerance();
            if (angleConditionWriter->IsAngleToleranceParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ANGLE_TOLERANCE.c_str(), angleConditionWriter->GetParameterFromAngleTolerance().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ANGLE_TOLERANCE.c_str(), XmlExportHelper::ToXmlStringFromDouble( kAngleTolerance).c_str());
            }
            const auto kAngleType = angleConditionWriter->GetAngleType();
            if (angleConditionWriter->IsAngleTypeParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ANGLE_TYPE.c_str(), angleConditionWriter->GetParameterFromAngleType().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ANGLE_TYPE.c_str(), kAngleType.GetLiteral().c_str());
            }
            const auto kCoordinateSystem = angleConditionWriter->GetCoordinateSystem();
            if (!( kCoordinateSystem == CoordinateSystem::UNKNOWN) || angleConditionWriter->IsSetCoordinateSystem())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__COORDINATE_SYSTEM.c_str(), kCoordinateSystem.GetLiteral().c_str());
            }
			else if (angleConditionWriter->IsCoordinateSystemParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__COORDINATE_SYSTEM.c_str(), angleConditionWriter->GetParameterFromCoordinateSystem().c_str());
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        void OpenScenarioXmlExporter::FillAnimationActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IAnimationActionWriter> animationActionWriter)
        {
            ...
            if (!( kAnimationDuration == 0) || animationActionWriter->IsSetAnimationDuration())
            {
            ...
            if (!( kLoop == false) || animationActionWriter->IsSetLoop())
            {
        ...
        void OpenScenarioXmlExporter::FillAnimationFileNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IAnimationFileWriter> animationFileWriter)
        {
            ...
            if (!( kTimeOffset == 0) || animationFileWriter->IsSetTimeOffset())
            {
        ...
        void OpenScenarioXmlExporter::FillAssignControllerActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IAssignControllerActionWriter> assignControllerActionWriter)
        {
            ...
            if (!( kActivateAnimation == false) || assignControllerActionWriter->IsSetActivateAnimation())
            {
            ...
            if (!( kActivateLighting == false) || assignControllerActionWriter->IsSetActivateLighting())
            {
            ...
            const auto kObjectController = assignControllerActionWriter->GetWriterObjectController();
        ...
        void OpenScenarioXmlExporter::FillBrakeNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IBrakeWriter> brakeWriter)
        {
            ...
            if (!( kMaxRate == 0) || brakeWriter->IsSetMaxRate())
            {
        ...
        void OpenScenarioXmlExporter::FillCatalogNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ICatalogWriter> catalogWriter)
        {
            ...
            if (catalogWriter->IsNameParameterized())
            {
        ...
        void OpenScenarioXmlExporter::FillClothoidSplineNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IClothoidSplineWriter> clothoidSplineWriter)
        {
            // Add Attributes (Parameters)
            const auto kTimeEnd = clothoidSplineWriter->GetTimeEnd();
            if (!( kTimeEnd == 0) || clothoidSplineWriter->IsSetTimeEnd())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__TIME_END.c_str(), XmlExportHelper::ToXmlStringFromDouble( kTimeEnd).c_str());
            }
			else if (clothoidSplineWriter->IsTimeEndParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__TIME_END.c_str(), clothoidSplineWriter->GetParameterFromTimeEnd().c_str());
            }
            const auto kSegments = clothoidSplineWriter->GetWriterSegments();
            if (!kSegments.empty())
            {
                for (auto segmentsWriterItem : kSegments)
                {
                    tinyxml2::XMLNode* segmentsElement = document->NewElement(OSC_CONSTANTS::ELEMENT__CLOTHOID_SPLINE_SEGMENT.c_str());
                    elementNode->InsertEndChild(segmentsElement);
                    FillClothoidSplineSegmentNode(document, segmentsElement, segmentsWriterItem);
                    
                }
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        void OpenScenarioXmlExporter::FillClothoidSplineSegmentNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IClothoidSplineSegmentWriter> clothoidSplineSegmentWriter)
        {
            // Add Attributes (Parameters)
            const auto kCurvatureEnd = clothoidSplineSegmentWriter->GetCurvatureEnd();
            if (clothoidSplineSegmentWriter->IsCurvatureEndParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__CURVATURE_END.c_str(), clothoidSplineSegmentWriter->GetParameterFromCurvatureEnd().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__CURVATURE_END.c_str(), XmlExportHelper::ToXmlStringFromDouble( kCurvatureEnd).c_str());
            }
            const auto kCurvatureStart = clothoidSplineSegmentWriter->GetCurvatureStart();
            if (clothoidSplineSegmentWriter->IsCurvatureStartParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__CURVATURE_START.c_str(), clothoidSplineSegmentWriter->GetParameterFromCurvatureStart().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__CURVATURE_START.c_str(), XmlExportHelper::ToXmlStringFromDouble( kCurvatureStart).c_str());
            }
            const auto kHOffset = clothoidSplineSegmentWriter->GetHOffset();
            if (!( kHOffset == 0) || clothoidSplineSegmentWriter->IsSetHOffset())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__H_OFFSET.c_str(), XmlExportHelper::ToXmlStringFromDouble( kHOffset).c_str());
            }
			else if (clothoidSplineSegmentWriter->IsHOffsetParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__H_OFFSET.c_str(), clothoidSplineSegmentWriter->GetParameterFromHOffset().c_str());
            }
            const auto kLength = clothoidSplineSegmentWriter->GetLength();
            if (clothoidSplineSegmentWriter->IsLengthParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__LENGTH.c_str(), clothoidSplineSegmentWriter->GetParameterFromLength().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__LENGTH.c_str(), XmlExportHelper::ToXmlStringFromDouble( kLength).c_str());
            }
            const auto kTimeStart = clothoidSplineSegmentWriter->GetTimeStart();
            if (!( kTimeStart == 0) || clothoidSplineSegmentWriter->IsSetTimeStart())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__TIME_START.c_str(), XmlExportHelper::ToXmlStringFromDouble( kTimeStart).c_str());
            }
			else if (clothoidSplineSegmentWriter->IsTimeStartParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__TIME_START.c_str(), clothoidSplineSegmentWriter->GetParameterFromTimeStart().c_str());
            }
            const auto kPositionStart = clothoidSplineSegmentWriter->GetWriterPositionStart();
            if (kPositionStart)
            {
                tinyxml2::XMLNode* positionStartElement = document->NewElement(OSC_CONSTANTS::ELEMENT__POSITION_START.c_str());
                elementNode->InsertEndChild(positionStartElement);
                FillPositionNode(document, positionStartElement, kPositionStart);
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        ...
        void OpenScenarioXmlExporter::FillConnectTrailerActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IConnectTrailerActionWriter> connectTrailerActionWriter)
        {
            // Add Attributes (Parameters)
            const auto kTrailerRef = connectTrailerActionWriter->GetTrailerRef();
            if (kTrailerRef)
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__TRAILER_REF.c_str(), XmlExportHelper::ToXmlStringFromString(kTrailerRef->GetNameRef()).c_str());
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        ...
        void OpenScenarioXmlExporter::FillControllerNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IControllerWriter> controllerWriter)
        {
            ...
            if (!( kControllerType == ControllerType::UNKNOWN) || controllerWriter->IsSetControllerType())
            {
        ...
        void OpenScenarioXmlExporter::FillDisconnectTrailerActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IDisconnectTrailerActionWriter> disconnectTrailerActionWriter)
        {
            // Add Attributes (Parameters)
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        void OpenScenarioXmlExporter::FillDistanceConditionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IDistanceConditionWriter> distanceConditionWriter)
        {
            ...
            if (!( kRoutingAlgorithm == RoutingAlgorithm::UNKNOWN) || distanceConditionWriter->IsSetRoutingAlgorithm())
            {
        ...
        void OpenScenarioXmlExporter::FillDomeImageNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IDomeImageWriter> domeImageWriter)
        {
            ...
            if (!( kAzimuthOffset == 0) || domeImageWriter->IsSetAzimuthOffset())
            {
        ...
        void OpenScenarioXmlExporter::FillDynamicConstraintsNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IDynamicConstraintsWriter> dynamicConstraintsWriter)
        {
            ...
            if (!( kMaxAccelerationRate == 0) || dynamicConstraintsWriter->IsSetMaxAccelerationRate())
            {
            ...
            if (!( kMaxDecelerationRate == 0) || dynamicConstraintsWriter->IsSetMaxDecelerationRate())
            {
        ...
        void OpenScenarioXmlExporter::FillEntityConditionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IEntityConditionWriter> entityConditionWriter)
        {
            ...
            const auto kAngleCondition = entityConditionWriter->GetWriterAngleCondition();
            if (kAngleCondition)
            {
                tinyxml2::XMLNode* angleConditionElement = document->NewElement(OSC_CONSTANTS::ELEMENT__ANGLE_CONDITION.c_str());
                elementNode->InsertEndChild(angleConditionElement);
                FillAngleConditionNode(document, angleConditionElement, kAngleCondition);
            }
            const auto kRelativeAngleCondition = entityConditionWriter->GetWriterRelativeAngleCondition();
            if (kRelativeAngleCondition)
            {
                tinyxml2::XMLNode* relativeAngleConditionElement = document->NewElement(OSC_CONSTANTS::ELEMENT__RELATIVE_ANGLE_CONDITION.c_str());
                elementNode->InsertEndChild(relativeAngleConditionElement);
                FillRelativeAngleConditionNode(document, relativeAngleConditionElement, kRelativeAngleCondition);
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        void OpenScenarioXmlExporter::FillEntityDistributionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IEntityDistributionWriter> entityDistributionWriter)
        {
            // Add Attributes (Parameters)
            const auto kEntityDistributionEntry = entityDistributionWriter->GetWriterEntityDistributionEntry();
            if (!kEntityDistributionEntry.empty())
            {
                for (auto entityDistributionEntryWriterItem : kEntityDistributionEntry)
                {
                    tinyxml2::XMLNode* entityDistributionEntryElement = document->NewElement(OSC_CONSTANTS::ELEMENT__ENTITY_DISTRIBUTION_ENTRY.c_str());
                    elementNode->InsertEndChild(entityDistributionEntryElement);
                    FillEntityDistributionEntryNode(document, entityDistributionEntryElement, entityDistributionEntryWriterItem);
                    
                }
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        void OpenScenarioXmlExporter::FillEntityDistributionEntryNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IEntityDistributionEntryWriter> entityDistributionEntryWriter)
        {
            // Add Attributes (Parameters)
            const auto kWeight = entityDistributionEntryWriter->GetWeight();
            if (entityDistributionEntryWriter->IsWeightParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__WEIGHT.c_str(), entityDistributionEntryWriter->GetParameterFromWeight().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__WEIGHT.c_str(), XmlExportHelper::ToXmlStringFromDouble( kWeight).c_str());
            }
            const auto kScenarioObjectTemplate = entityDistributionEntryWriter->GetWriterScenarioObjectTemplate();
            if (kScenarioObjectTemplate)
            {
                tinyxml2::XMLNode* scenarioObjectTemplateElement = document->NewElement(OSC_CONSTANTS::ELEMENT__SCENARIO_OBJECT_TEMPLATE.c_str());
                elementNode->InsertEndChild(scenarioObjectTemplateElement);
                FillScenarioObjectTemplateNode(document, scenarioObjectTemplateElement, kScenarioObjectTemplate);
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        ...
        void OpenScenarioXmlExporter::FillGeoPositionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IGeoPositionWriter> geoPositionWriter)
        {
            ...
            if (!( kAltitude == 0) || geoPositionWriter->IsSetAltitude())
            {
            ...
            if (!( kLatitude == 0) || geoPositionWriter->IsSetLatitude())
            {
            ...
            if (!( kLatitudeDeg == 0) || geoPositionWriter->IsSetLatitudeDeg())
            {
            ...
            if (!( kLongitude == 0) || geoPositionWriter->IsSetLongitude())
            {
            ...
            if (!( kLongitudeDeg == 0) || geoPositionWriter->IsSetLongitudeDeg())
            {
            ...
            const auto kVerticalRoadSelection = geoPositionWriter->GetVerticalRoadSelection();
            if (!( kVerticalRoadSelection == 0) || geoPositionWriter->IsSetVerticalRoadSelection())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__VERTICAL_ROAD_SELECTION.c_str(), XmlExportHelper::ToXmlStringFromInt( kVerticalRoadSelection).c_str());
            }
			else if (geoPositionWriter->IsVerticalRoadSelectionParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__VERTICAL_ROAD_SELECTION.c_str(), geoPositionWriter->GetParameterFromVerticalRoadSelection().c_str());
            }
        ...
        void OpenScenarioXmlExporter::FillGlobalActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IGlobalActionWriter> globalActionWriter)
        {
            ...
            const auto kSetMonitorAction = globalActionWriter->GetWriterSetMonitorAction();
            if (kSetMonitorAction)
            {
                tinyxml2::XMLNode* setMonitorActionElement = document->NewElement(OSC_CONSTANTS::ELEMENT__SET_MONITOR_ACTION.c_str());
                elementNode->InsertEndChild(setMonitorActionElement);
                FillSetMonitorActionNode(document, setMonitorActionElement, kSetMonitorAction);
            }
        ...
        void OpenScenarioXmlExporter::FillLaneNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ILaneWriter> laneWriter)
        {
            // Add Attributes (Parameters)
            const auto kId = laneWriter->GetId();
            if (laneWriter->IsIdParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ID.c_str(), laneWriter->GetParameterFromId().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ID.c_str(), XmlExportHelper::ToXmlStringFromInt( kId).c_str());
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        ...
        void OpenScenarioXmlExporter::FillLightStateNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ILightStateWriter> lightStateWriter)
        {
            ...
            if (!( kFlashingOffDuration == 0) || lightStateWriter->IsSetFlashingOffDuration())
            {
            ...
            if (!( kFlashingOnDuration == 0) || lightStateWriter->IsSetFlashingOnDuration())
            {
            ...
            if (!( kLuminousIntensity == 0) || lightStateWriter->IsSetLuminousIntensity())
            {
        ...
        void OpenScenarioXmlExporter::FillLightStateActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ILightStateActionWriter> lightStateActionWriter)
        {
            ...
            if (!( kTransitionTime == 0) || lightStateActionWriter->IsSetTransitionTime())
            {
        ...
        void OpenScenarioXmlExporter::FillLogNormalDistributionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ILogNormalDistributionWriter> logNormalDistributionWriter)
        {
            // Add Attributes (Parameters)
            const auto kExpectedValue = logNormalDistributionWriter->GetExpectedValue();
            if (logNormalDistributionWriter->IsExpectedValueParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__EXPECTED_VALUE.c_str(), logNormalDistributionWriter->GetParameterFromExpectedValue().c_str());
            }
            else
            {
            ...
            const auto kVariance = logNormalDistributionWriter->GetVariance();
            if (logNormalDistributionWriter->IsVarianceParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__VARIANCE.c_str(), logNormalDistributionWriter->GetParameterFromVariance().c_str());
            }
            else
            {
            ...
            const auto kRange = logNormalDistributionWriter->GetWriterRange();
            if (kRange)
            {
                tinyxml2::XMLNode* rangeElement = document->NewElement(OSC_CONSTANTS::ELEMENT__RANGE.c_str());
                elementNode->InsertEndChild(rangeElement);
        ...
        void OpenScenarioXmlExporter::FillMonitorDeclarationNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IMonitorDeclarationWriter> monitorDeclarationWriter)
        {
            // Add Attributes (Parameters)
            const auto kName = monitorDeclarationWriter->GetName();
            if (monitorDeclarationWriter->IsNameParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__NAME.c_str(), monitorDeclarationWriter->GetParameterFromName().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__NAME.c_str(), XmlExportHelper::ToXmlStringFromString( kName).c_str());
            }
            const auto kValue = monitorDeclarationWriter->GetValue();
            if (monitorDeclarationWriter->IsValueParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__VALUE.c_str(), monitorDeclarationWriter->GetParameterFromValue().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__VALUE.c_str(), XmlExportHelper::ToXmlStringFromBoolean( kValue).c_str());
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        ...
        {
            // Add Attributes (Parameters)
            const auto kName = objectControllerWriter->GetName();
            if (!( kName.empty()) || objectControllerWriter->IsSetName())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__NAME.c_str(), XmlExportHelper::ToXmlStringFromString( kName).c_str());
            }
			else if (objectControllerWriter->IsNameParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__NAME.c_str(), objectControllerWriter->GetParameterFromName().c_str());
            }
        ...
        void OpenScenarioXmlExporter::FillOverrideBrakeActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IOverrideBrakeActionWriter> overrideBrakeActionWriter)
        {
            ...
            if (!( kValue == 0) || overrideBrakeActionWriter->IsSetValue())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__VALUE.c_str(), XmlExportHelper::ToXmlStringFromDouble( kValue).c_str());
            }
        ...
        void OpenScenarioXmlExporter::FillOverrideClutchActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IOverrideClutchActionWriter> overrideClutchActionWriter)
        {
            ...
            if (!( kMaxRate == 0) || overrideClutchActionWriter->IsSetMaxRate())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__MAX_RATE.c_str(), XmlExportHelper::ToXmlStringFromDouble( kMaxRate).c_str());
            }
        ...
        void OpenScenarioXmlExporter::FillOverrideGearActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IOverrideGearActionWriter> overrideGearActionWriter)
        {
            ...
            if (!( kNumber == 0) || overrideGearActionWriter->IsSetNumber())
            {
        ...
        void OpenScenarioXmlExporter::FillOverrideParkingBrakeActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IOverrideParkingBrakeActionWriter> overrideParkingBrakeActionWriter)
        {
            ...
            if (!( kValue == 0) || overrideParkingBrakeActionWriter->IsSetValue())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__VALUE.c_str(), XmlExportHelper::ToXmlStringFromDouble( kValue).c_str());
            }
        ...
        void OpenScenarioXmlExporter::FillOverrideSteeringWheelActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IOverrideSteeringWheelActionWriter> overrideSteeringWheelActionWriter)
        {
            ...
            if (!( kMaxRate == 0) || overrideSteeringWheelActionWriter->IsSetMaxRate())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__MAX_RATE.c_str(), XmlExportHelper::ToXmlStringFromDouble( kMaxRate).c_str());
            }
            ...
            if (!( kMaxTorque == 0) || overrideSteeringWheelActionWriter->IsSetMaxTorque())
            {
        ...
        void OpenScenarioXmlExporter::FillOverrideThrottleActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IOverrideThrottleActionWriter> overrideThrottleActionWriter)
        {
            ...
            if (!( kMaxRate == 0) || overrideThrottleActionWriter->IsSetMaxRate())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__MAX_RATE.c_str(), XmlExportHelper::ToXmlStringFromDouble( kMaxRate).c_str());
            }
        ...
        void OpenScenarioXmlExporter::FillPedestrianNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IPedestrianWriter> pedestrianWriter)
        {
            ...
            if (!( kRole == Role::UNKNOWN) || pedestrianWriter->IsSetRole())
            {
        ...
        void OpenScenarioXmlExporter::FillPedestrianAnimationNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IPedestrianAnimationWriter> pedestrianAnimationWriter)
        {
            ...
            if (!( kMotion == PedestrianMotionType::UNKNOWN) || pedestrianAnimationWriter->IsSetMotion())
            {
            ...
            if (!( kUserDefinedPedestrianAnimation.empty()) || pedestrianAnimationWriter->IsSetUserDefinedPedestrianAnimation())
            {
        ...
        void OpenScenarioXmlExporter::FillPerformanceNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IPerformanceWriter> performanceWriter)
        {
            ...
            if (!( kMaxAccelerationRate == 0) || performanceWriter->IsSetMaxAccelerationRate())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__MAX_ACCELERATION_RATE.c_str(), XmlExportHelper::ToXmlStringFromDouble( kMaxAccelerationRate).c_str());
            }
            ...
            if (!( kMaxDecelerationRate == 0) || performanceWriter->IsSetMaxDecelerationRate())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__MAX_DECELERATION_RATE.c_str(), XmlExportHelper::ToXmlStringFromDouble( kMaxDecelerationRate).c_str());
            }
        ...
        void OpenScenarioXmlExporter::FillPhaseNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IPhaseWriter> phaseWriter)
        {
            ...
            const auto kTrafficSignalGroupState = phaseWriter->GetWriterTrafficSignalGroupState();
            if (kTrafficSignalGroupState)
            {
                tinyxml2::XMLNode* trafficSignalGroupStateElement = document->NewElement(OSC_CONSTANTS::ELEMENT__TRAFFIC_SIGNAL_GROUP_STATE.c_str());
                elementNode->InsertEndChild(trafficSignalGroupStateElement);
                FillTrafficSignalGroupStateNode(document, trafficSignalGroupStateElement, kTrafficSignalGroupState);
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        ...
        void OpenScenarioXmlExporter::FillPolygonNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IPolygonWriter> polygonWriter)
        {
            // Add Attributes (Parameters)
            const auto kPositions = polygonWriter->GetWriterPositions();
            if (!kPositions.empty())
            {
        ...
        void OpenScenarioXmlExporter::FillPrivateActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IPrivateActionWriter> privateActionWriter)
        {
            ...
            const auto kTrailerAction = privateActionWriter->GetWriterTrailerAction();
            if (kTrailerAction)
            {
                tinyxml2::XMLNode* trailerActionElement = document->NewElement(OSC_CONSTANTS::ELEMENT__TRAILER_ACTION.c_str());
                elementNode->InsertEndChild(trailerActionElement);
                FillTrailerActionNode(document, trailerActionElement, kTrailerAction);
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        ...
        void OpenScenarioXmlExporter::FillRandomRouteActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IRandomRouteActionWriter> randomRouteActionWriter)
        {
            // Add Attributes (Parameters)
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        ...
        void OpenScenarioXmlExporter::FillRelativeAngleConditionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IRelativeAngleConditionWriter> relativeAngleConditionWriter)
        {
            // Add Attributes (Parameters)
            const auto kAngle = relativeAngleConditionWriter->GetAngle();
            if (relativeAngleConditionWriter->IsAngleParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ANGLE.c_str(), relativeAngleConditionWriter->GetParameterFromAngle().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ANGLE.c_str(), XmlExportHelper::ToXmlStringFromDouble( kAngle).c_str());
            }
            const auto kAngleTolerance = relativeAngleConditionWriter->GetAngleTolerance();
            if (relativeAngleConditionWriter->IsAngleToleranceParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ANGLE_TOLERANCE.c_str(), relativeAngleConditionWriter->GetParameterFromAngleTolerance().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ANGLE_TOLERANCE.c_str(), XmlExportHelper::ToXmlStringFromDouble( kAngleTolerance).c_str());
            }
            const auto kAngleType = relativeAngleConditionWriter->GetAngleType();
            if (relativeAngleConditionWriter->IsAngleTypeParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ANGLE_TYPE.c_str(), relativeAngleConditionWriter->GetParameterFromAngleType().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ANGLE_TYPE.c_str(), kAngleType.GetLiteral().c_str());
            }
            const auto kCoordinateSystem = relativeAngleConditionWriter->GetCoordinateSystem();
            if (!( kCoordinateSystem == CoordinateSystem::UNKNOWN) || relativeAngleConditionWriter->IsSetCoordinateSystem())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__COORDINATE_SYSTEM.c_str(), kCoordinateSystem.GetLiteral().c_str());
            }
			else if (relativeAngleConditionWriter->IsCoordinateSystemParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__COORDINATE_SYSTEM.c_str(), relativeAngleConditionWriter->GetParameterFromCoordinateSystem().c_str());
            }
            const auto kEntityRef = relativeAngleConditionWriter->GetEntityRef();
            if (kEntityRef)
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ENTITY_REF.c_str(), XmlExportHelper::ToXmlStringFromString(kEntityRef->GetNameRef()).c_str());
            }
        ...
        void OpenScenarioXmlExporter::FillRelativeClearanceConditionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IRelativeClearanceConditionWriter> relativeClearanceConditionWriter)
        {
            ...
            if (!( kDistanceBackward == 0) || relativeClearanceConditionWriter->IsSetDistanceBackward())
            {
            ...
            if (!( kDistanceForward == 0) || relativeClearanceConditionWriter->IsSetDistanceForward())
            {
        ...
        void OpenScenarioXmlExporter::FillRelativeDistanceConditionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IRelativeDistanceConditionWriter> relativeDistanceConditionWriter)
        {
            ...
            if (!( kRoutingAlgorithm == RoutingAlgorithm::UNKNOWN) || relativeDistanceConditionWriter->IsSetRoutingAlgorithm())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ROUTING_ALGORITHM.c_str(), kRoutingAlgorithm.GetLiteral().c_str());
            }
        ...
        void OpenScenarioXmlExporter::FillRelativeLaneRangeNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IRelativeLaneRangeWriter> relativeLaneRangeWriter)
        {
            ...
            if (!( kFrom == 0) || relativeLaneRangeWriter->IsSetFrom())
            {
            ...
            if (!( kTo == 0) || relativeLaneRangeWriter->IsSetTo())
            {
        ...
        void OpenScenarioXmlExporter::FillRelativeSpeedConditionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IRelativeSpeedConditionWriter> relativeSpeedConditionWriter)
        {
            ...
            if (!( kDirection == DirectionalDimension::UNKNOWN) || relativeSpeedConditionWriter->IsSetDirection())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__DIRECTION.c_str(), kDirection.GetLiteral().c_str());
            }
        ...
        void OpenScenarioXmlExporter::FillRoadConditionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IRoadConditionWriter> roadConditionWriter)
        {
            ...
            if (!( kWetness == Wetness::UNKNOWN) || roadConditionWriter->IsSetWetness())
            {
        ...
        void OpenScenarioXmlExporter::FillRoadCursorNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IRoadCursorWriter> roadCursorWriter)
        {
            // Add Attributes (Parameters)
            const auto kRoadId = roadCursorWriter->GetRoadId();
            if (roadCursorWriter->IsRoadIdParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ROAD_ID.c_str(), roadCursorWriter->GetParameterFromRoadId().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ROAD_ID.c_str(), XmlExportHelper::ToXmlStringFromString( kRoadId).c_str());
            }
            const auto kS = roadCursorWriter->GetS();
            if (!( kS == 0) || roadCursorWriter->IsSetS())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__S.c_str(), XmlExportHelper::ToXmlStringFromDouble( kS).c_str());
            }
			else if (roadCursorWriter->IsSParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__S.c_str(), roadCursorWriter->GetParameterFromS().c_str());
            }
            const auto kLane = roadCursorWriter->GetWriterLane();
            if (!kLane.empty())
            {
                for (auto laneWriterItem : kLane)
                {
                    tinyxml2::XMLNode* laneElement = document->NewElement(OSC_CONSTANTS::ELEMENT__LANE.c_str());
                    elementNode->InsertEndChild(laneElement);
                    FillLaneNode(document, laneElement, laneWriterItem);
        ...
        void OpenScenarioXmlExporter::FillRoadRangeNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IRoadRangeWriter> roadRangeWriter)
        {
            // Add Attributes (Parameters)
            const auto kLength = roadRangeWriter->GetLength();
            if (!( kLength == 0) || roadRangeWriter->IsSetLength())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__LENGTH.c_str(), XmlExportHelper::ToXmlStringFromDouble( kLength).c_str());
            }
			else if (roadRangeWriter->IsLengthParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__LENGTH.c_str(), roadRangeWriter->GetParameterFromLength().c_str());
            }
            const auto kRoadCursor = roadRangeWriter->GetWriterRoadCursor();
            if (!kRoadCursor.empty())
            {
                for (auto roadCursorWriterItem : kRoadCursor)
                {
                    tinyxml2::XMLNode* roadCursorElement = document->NewElement(OSC_CONSTANTS::ELEMENT__ROAD_CURSOR.c_str());
                    elementNode->InsertEndChild(roadCursorElement);
                    FillRoadCursorNode(document, roadCursorElement, roadCursorWriterItem);
        ...
        void OpenScenarioXmlExporter::FillRoutingActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IRoutingActionWriter> routingActionWriter)
        {
            ...
            const auto kRandomRouteAction = routingActionWriter->GetWriterRandomRouteAction();
            if (kRandomRouteAction)
            {
                tinyxml2::XMLNode* randomRouteActionElement = document->NewElement(OSC_CONSTANTS::ELEMENT__RANDOM_ROUTE_ACTION.c_str());
                elementNode->InsertEndChild(randomRouteActionElement);
                FillRandomRouteActionNode(document, randomRouteActionElement, kRandomRouteAction);
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        void OpenScenarioXmlExporter::FillScenarioDefinitionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IScenarioDefinitionWriter> scenarioDefinitionWriter)
        {
            ...
            const auto kMonitorDeclarations = scenarioDefinitionWriter->GetWriterMonitorDeclarations();
            if (!kMonitorDeclarations.empty())
            {
                tinyxml2::XMLNode* monitorDeclarationsWrapperElement = document->NewElement(OSC_CONSTANTS::ELEMENT__MONITOR_DECLARATIONS.c_str());
                elementNode->InsertEndChild(monitorDeclarationsWrapperElement);
                for (auto monitorDeclarationsWriterItem : kMonitorDeclarations)
                {
                    tinyxml2::XMLNode* monitorDeclarationsElement = document->NewElement(OSC_CONSTANTS::ELEMENT__MONITOR_DECLARATION.c_str());
                    monitorDeclarationsWrapperElement->InsertEndChild(monitorDeclarationsElement);
                    FillMonitorDeclarationNode(document, monitorDeclarationsElement, monitorDeclarationsWriterItem);
                }
            }
        ...
        void OpenScenarioXmlExporter::FillScenarioObjectTemplateNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IScenarioObjectTemplateWriter> scenarioObjectTemplateWriter)
        {
            // Add Attributes (Parameters)
            const auto kEntitiyObject = scenarioObjectTemplateWriter->GetWriterEntitiyObject();
            if (kEntitiyObject)
            {
				FillEntityObjectNode(document, elementNode, kEntitiyObject);
            }
            const auto kObjectController = scenarioObjectTemplateWriter->GetWriterObjectController();
            if (!kObjectController.empty())
            {
                for (auto objectControllerWriterItem : kObjectController)
                {
        ...
        void OpenScenarioXmlExporter::FillSetMonitorActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ISetMonitorActionWriter> setMonitorActionWriter)
        {
            // Add Attributes (Parameters)
            const auto kMonitorRef = setMonitorActionWriter->GetMonitorRef();
            if (kMonitorRef)
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__MONITOR_REF.c_str(), XmlExportHelper::ToXmlStringFromString(kMonitorRef->GetNameRef()).c_str());
            }
            const auto kValue = setMonitorActionWriter->GetValue();
            if (setMonitorActionWriter->IsValueParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__VALUE.c_str(), setMonitorActionWriter->GetParameterFromValue().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__VALUE.c_str(), XmlExportHelper::ToXmlStringFromBoolean( kValue).c_str());
        ...
        void OpenScenarioXmlExporter::FillShapeNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IShapeWriter> shapeWriter)
        {
            ...
            const auto kClothoidSpline = shapeWriter->GetWriterClothoidSpline();
            if (kClothoidSpline)
            {
                tinyxml2::XMLNode* clothoidSplineElement = document->NewElement(OSC_CONSTANTS::ELEMENT__CLOTHOID_SPLINE.c_str());
                elementNode->InsertEndChild(clothoidSplineElement);
                FillClothoidSplineNode(document, clothoidSplineElement, kClothoidSpline);
            }
        ...
        void OpenScenarioXmlExporter::FillSpeedConditionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ISpeedConditionWriter> speedConditionWriter)
        {
            ...
            if (!( kDirection == DirectionalDimension::UNKNOWN) || speedConditionWriter->IsSetDirection())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__DIRECTION.c_str(), kDirection.GetLiteral().c_str());
            }
        ...
        void OpenScenarioXmlExporter::FillSpeedProfileEntryNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ISpeedProfileEntryWriter> speedProfileEntryWriter)
        {
            ...
            if (!( kTime == 0) || speedProfileEntryWriter->IsSetTime())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__TIME.c_str(), XmlExportHelper::ToXmlStringFromDouble( kTime).c_str());
            }
        ...
        void OpenScenarioXmlExporter::FillStochasticDistributionTypeNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IStochasticDistributionTypeWriter> stochasticDistributionTypeWriter)
        {
            ...
            const auto kLogNormalDistribution = stochasticDistributionTypeWriter->GetWriterLogNormalDistribution();
            if (kLogNormalDistribution)
            {
                tinyxml2::XMLNode* logNormalDistributionElement = document->NewElement(OSC_CONSTANTS::ELEMENT__LOG_NORMAL_DISTRIBUTION.c_str());
                elementNode->InsertEndChild(logNormalDistributionElement);
                FillLogNormalDistributionNode(document, logNormalDistributionElement, kLogNormalDistribution);
            }
        ...
        void OpenScenarioXmlExporter::FillSunNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ISunWriter> sunWriter)
        {
            ...
            if (!( kIlluminance == 0) || sunWriter->IsSetIlluminance())
            {
            ...
            if (!( kIntensity == 0) || sunWriter->IsSetIntensity())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__INTENSITY.c_str(), XmlExportHelper::ToXmlStringFromDouble( kIntensity).c_str());
            }
        ...
        void OpenScenarioXmlExporter::FillTimeHeadwayConditionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ITimeHeadwayConditionWriter> timeHeadwayConditionWriter)
        {
            ...
            if (!( kRoutingAlgorithm == RoutingAlgorithm::UNKNOWN) || timeHeadwayConditionWriter->IsSetRoutingAlgorithm())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ROUTING_ALGORITHM.c_str(), kRoutingAlgorithm.GetLiteral().c_str());
            }
        ...
        void OpenScenarioXmlExporter::FillTimeToCollisionConditionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ITimeToCollisionConditionWriter> timeToCollisionConditionWriter)
        {
            ...
            if (!( kRoutingAlgorithm == RoutingAlgorithm::UNKNOWN) || timeToCollisionConditionWriter->IsSetRoutingAlgorithm())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ROUTING_ALGORITHM.c_str(), kRoutingAlgorithm.GetLiteral().c_str());
            }
        ...
        void OpenScenarioXmlExporter::FillTrafficActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ITrafficActionWriter> trafficActionWriter)
        {
            ...
            const auto kTrafficAreaAction = trafficActionWriter->GetWriterTrafficAreaAction();
            if (kTrafficAreaAction)
            {
                tinyxml2::XMLNode* trafficAreaActionElement = document->NewElement(OSC_CONSTANTS::ELEMENT__TRAFFIC_AREA_ACTION.c_str());
                elementNode->InsertEndChild(trafficAreaActionElement);
                FillTrafficAreaActionNode(document, trafficAreaActionElement, kTrafficAreaAction);
            }
        ...
        void OpenScenarioXmlExporter::FillTrafficAreaNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ITrafficAreaWriter> trafficAreaWriter)
        {
            // Add Attributes (Parameters)
            const auto kPolygon = trafficAreaWriter->GetWriterPolygon();
            if (kPolygon)
            {
                tinyxml2::XMLNode* polygonElement = document->NewElement(OSC_CONSTANTS::ELEMENT__POLYGON.c_str());
                elementNode->InsertEndChild(polygonElement);
                FillPolygonNode(document, polygonElement, kPolygon);
            }
            const auto kRoadRange = trafficAreaWriter->GetWriterRoadRange();
            if (!kRoadRange.empty())
            {
                for (auto roadRangeWriterItem : kRoadRange)
                {
                    tinyxml2::XMLNode* roadRangeElement = document->NewElement(OSC_CONSTANTS::ELEMENT__ROAD_RANGE.c_str());
                    elementNode->InsertEndChild(roadRangeElement);
                    FillRoadRangeNode(document, roadRangeElement, roadRangeWriterItem);
                    
                }
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        void OpenScenarioXmlExporter::FillTrafficAreaActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ITrafficAreaActionWriter> trafficAreaActionWriter)
        {
            // Add Attributes (Parameters)
            const auto kContinuous = trafficAreaActionWriter->GetContinuous();
            if (trafficAreaActionWriter->IsContinuousParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__CONTINUOUS.c_str(), trafficAreaActionWriter->GetParameterFromContinuous().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__CONTINUOUS.c_str(), XmlExportHelper::ToXmlStringFromBoolean( kContinuous).c_str());
            }
            const auto kNumberOfEntities = trafficAreaActionWriter->GetNumberOfEntities();
            if (trafficAreaActionWriter->IsNumberOfEntitiesParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__NUMBER_OF_ENTITIES.c_str(), trafficAreaActionWriter->GetParameterFromNumberOfEntities().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__NUMBER_OF_ENTITIES.c_str(), XmlExportHelper::ToXmlStringFromUnsignedInt( kNumberOfEntities).c_str());
            }
            const auto kTrafficDistribution = trafficAreaActionWriter->GetWriterTrafficDistribution();
            if (kTrafficDistribution)
            {
                tinyxml2::XMLNode* trafficDistributionElement = document->NewElement(OSC_CONSTANTS::ELEMENT__TRAFFIC_DISTRIBUTION.c_str());
                elementNode->InsertEndChild(trafficDistributionElement);
                FillTrafficDistributionNode(document, trafficDistributionElement, kTrafficDistribution);
            }
            const auto kTrafficArea = trafficAreaActionWriter->GetWriterTrafficArea();
            if (kTrafficArea)
            {
                tinyxml2::XMLNode* trafficAreaElement = document->NewElement(OSC_CONSTANTS::ELEMENT__TRAFFIC_AREA.c_str());
                elementNode->InsertEndChild(trafficAreaElement);
                FillTrafficAreaNode(document, trafficAreaElement, kTrafficArea);
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        ...
        void OpenScenarioXmlExporter::FillTrafficDistributionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ITrafficDistributionWriter> trafficDistributionWriter)
        {
            // Add Attributes (Parameters)
            const auto kTrafficDistributionEntry = trafficDistributionWriter->GetWriterTrafficDistributionEntry();
            if (!kTrafficDistributionEntry.empty())
            {
                for (auto trafficDistributionEntryWriterItem : kTrafficDistributionEntry)
                {
                    tinyxml2::XMLNode* trafficDistributionEntryElement = document->NewElement(OSC_CONSTANTS::ELEMENT__TRAFFIC_DISTRIBUTION_ENTRY.c_str());
                    elementNode->InsertEndChild(trafficDistributionEntryElement);
                    FillTrafficDistributionEntryNode(document, trafficDistributionEntryElement, trafficDistributionEntryWriterItem);
                    
                }
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        void OpenScenarioXmlExporter::FillTrafficDistributionEntryNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ITrafficDistributionEntryWriter> trafficDistributionEntryWriter)
        {
            // Add Attributes (Parameters)
            const auto kWeight = trafficDistributionEntryWriter->GetWeight();
            if (trafficDistributionEntryWriter->IsWeightParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__WEIGHT.c_str(), trafficDistributionEntryWriter->GetParameterFromWeight().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__WEIGHT.c_str(), XmlExportHelper::ToXmlStringFromDouble( kWeight).c_str());
            }
            const auto kEntityDistribution = trafficDistributionEntryWriter->GetWriterEntityDistribution();
            if (kEntityDistribution)
            {
                tinyxml2::XMLNode* entityDistributionElement = document->NewElement(OSC_CONSTANTS::ELEMENT__ENTITY_DISTRIBUTION.c_str());
                elementNode->InsertEndChild(entityDistributionElement);
                FillEntityDistributionNode(document, entityDistributionElement, kEntityDistribution);
            }
            const auto kProperties = trafficDistributionEntryWriter->GetWriterProperties();
            if (kProperties)
            {
                tinyxml2::XMLNode* propertiesElement = document->NewElement(OSC_CONSTANTS::ELEMENT__PROPERTIES.c_str());
                elementNode->InsertEndChild(propertiesElement);
        ...
        void OpenScenarioXmlExporter::FillTrafficSourceActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ITrafficSourceActionWriter> trafficSourceActionWriter)
        {
            ...
            if (!( kSpeed == 0) || trafficSourceActionWriter->IsSetSpeed())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__SPEED.c_str(), XmlExportHelper::ToXmlStringFromDouble( kSpeed).c_str());
            }
            ...
            const auto kTrafficDistribution = trafficSourceActionWriter->GetWriterTrafficDistribution();
            if (kTrafficDistribution)
            {
                tinyxml2::XMLNode* trafficDistributionElement = document->NewElement(OSC_CONSTANTS::ELEMENT__TRAFFIC_DISTRIBUTION.c_str());
                elementNode->InsertEndChild(trafficDistributionElement);
        ...
        void OpenScenarioXmlExporter::FillTrafficSwarmActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ITrafficSwarmActionWriter> trafficSwarmActionWriter)
        {
            ...
            const auto kTrafficDistribution = trafficSwarmActionWriter->GetWriterTrafficDistribution();
            if (kTrafficDistribution)
            {
                tinyxml2::XMLNode* trafficDistributionElement = document->NewElement(OSC_CONSTANTS::ELEMENT__TRAFFIC_DISTRIBUTION.c_str());
                elementNode->InsertEndChild(trafficDistributionElement);
        ...
        void OpenScenarioXmlExporter::FillTrailerNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ITrailerWriter> trailerWriter)
        {
            // Add Attributes (Parameters)
            const auto kTrailer = trailerWriter->GetWriterTrailer();
            if (kTrailer)
            {
                tinyxml2::XMLNode* trailerElement = document->NewElement(OSC_CONSTANTS::ELEMENT__TRAILER.c_str());
                elementNode->InsertEndChild(trailerElement);
                FillScenarioObjectNode(document, trailerElement, kTrailer);
            }
            const auto kTrailerRef = trailerWriter->GetWriterTrailerRef();
            if (kTrailerRef)
            {
                tinyxml2::XMLNode* trailerRefElement = document->NewElement(OSC_CONSTANTS::ELEMENT__TRAILER_REF.c_str());
                elementNode->InsertEndChild(trailerRefElement);
                FillEntityRefNode(document, trailerRefElement, kTrailerRef);
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        void OpenScenarioXmlExporter::FillTrailerActionNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ITrailerActionWriter> trailerActionWriter)
        {
            // Add Attributes (Parameters)
            const auto kConnectTrailerAction = trailerActionWriter->GetWriterConnectTrailerAction();
            if (kConnectTrailerAction)
            {
                tinyxml2::XMLNode* connectTrailerActionElement = document->NewElement(OSC_CONSTANTS::ELEMENT__CONNECT_TRAILER_ACTION.c_str());
                elementNode->InsertEndChild(connectTrailerActionElement);
                FillConnectTrailerActionNode(document, connectTrailerActionElement, kConnectTrailerAction);
            }
            const auto kDisconnectTrailerAction = trailerActionWriter->GetWriterDisconnectTrailerAction();
            if (kDisconnectTrailerAction)
            {
                tinyxml2::XMLNode* disconnectTrailerActionElement = document->NewElement(OSC_CONSTANTS::ELEMENT__DISCONNECT_TRAILER_ACTION.c_str());
                elementNode->InsertEndChild(disconnectTrailerActionElement);
                FillDisconnectTrailerActionNode(document, disconnectTrailerActionElement, kDisconnectTrailerAction);
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        void OpenScenarioXmlExporter::FillTrailerCouplerNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ITrailerCouplerWriter> trailerCouplerWriter)
        {
            // Add Attributes (Parameters)
            const auto kDx = trailerCouplerWriter->GetDx();
            if (trailerCouplerWriter->IsDxParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__DX.c_str(), trailerCouplerWriter->GetParameterFromDx().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__DX.c_str(), XmlExportHelper::ToXmlStringFromDouble( kDx).c_str());
            }
            const auto kDz = trailerCouplerWriter->GetDz();
            if (!( kDz == 0) || trailerCouplerWriter->IsSetDz())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__DZ.c_str(), XmlExportHelper::ToXmlStringFromDouble( kDz).c_str());
            }
			else if (trailerCouplerWriter->IsDzParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__DZ.c_str(), trailerCouplerWriter->GetParameterFromDz().c_str());
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        void OpenScenarioXmlExporter::FillTrailerHitchNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ITrailerHitchWriter> trailerHitchWriter)
        {
            // Add Attributes (Parameters)
            const auto kDx = trailerHitchWriter->GetDx();
            if (trailerHitchWriter->IsDxParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__DX.c_str(), trailerHitchWriter->GetParameterFromDx().c_str());
            }
            else
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__DX.c_str(), XmlExportHelper::ToXmlStringFromDouble( kDx).c_str());
            }
            const auto kDz = trailerHitchWriter->GetDz();
            if (!( kDz == 0) || trailerHitchWriter->IsSetDz())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__DZ.c_str(), XmlExportHelper::ToXmlStringFromDouble( kDz).c_str());
            }
			else if (trailerHitchWriter->IsDzParameterized())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__DZ.c_str(), trailerHitchWriter->GetParameterFromDz().c_str());
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        ...
        void OpenScenarioXmlExporter::FillTransitionDynamicsNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<ITransitionDynamicsWriter> transitionDynamicsWriter)
        {
            ...
            if (!( kFollowingMode == FollowingMode::UNKNOWN) || transitionDynamicsWriter->IsSetFollowingMode())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__FOLLOWING_MODE.c_str(), kFollowingMode.GetLiteral().c_str());
            }
        ...
        void OpenScenarioXmlExporter::FillVehicleNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IVehicleWriter> vehicleWriter)
        {
            ...
            if (!( kRole == Role::UNKNOWN) || vehicleWriter->IsSetRole())
            {
                elementNode->ToElement()->SetAttribute(OSC_CONSTANTS::ATTRIBUTE__ROLE.c_str(), kRole.GetLiteral().c_str());
            }
            ...
            const auto kTrailerHitch = vehicleWriter->GetWriterTrailerHitch();
            if (kTrailerHitch)
            {
                tinyxml2::XMLNode* trailerHitchElement = document->NewElement(OSC_CONSTANTS::ELEMENT__TRAILER_HITCH.c_str());
                elementNode->InsertEndChild(trailerHitchElement);
                FillTrailerHitchNode(document, trailerHitchElement, kTrailerHitch);
            }
            const auto kTrailerCoupler = vehicleWriter->GetWriterTrailerCoupler();
            if (kTrailerCoupler)
            {
                tinyxml2::XMLNode* trailerCouplerElement = document->NewElement(OSC_CONSTANTS::ELEMENT__TRAILER_COUPLER.c_str());
                elementNode->InsertEndChild(trailerCouplerElement);
                FillTrailerCouplerNode(document, trailerCouplerElement, kTrailerCoupler);
            }
            const auto kTrailer = vehicleWriter->GetWriterTrailer();
            if (kTrailer)
            {
                tinyxml2::XMLNode* trailerElement = document->NewElement(OSC_CONSTANTS::ELEMENT__TRAILER.c_str());
                elementNode->InsertEndChild(trailerElement);
                FillTrailerNode(document, trailerElement, kTrailer);
            }
                // Add Children (Normal, Wrapped, Unwrapped, simpleContent);
            }
        ...
        void OpenScenarioXmlExporter::FillWeatherNode(std::shared_ptr<tinyxml2::XMLDocument> document, tinyxml2::XMLNode* elementNode, std::shared_ptr<IWeatherWriter> weatherWriter)
        {
            ...
            if (!( kFractionalCloudCover == FractionalCloudCover::UNKNOWN) || weatherWriter->IsSetFractionalCloudCover())
            {
