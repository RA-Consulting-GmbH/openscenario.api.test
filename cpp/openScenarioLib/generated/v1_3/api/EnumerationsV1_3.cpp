...
namespace NET_ASAM_OPENSCENARIO
{
    namespace v1_3
    {
	    	std::map<std::string, AngleType::AngleTypeEnum> AngleType::_stringToEnum = 
	        {
	            {"heading", HEADING},
	            {"pitch", PITCH},
	            {"roll", ROLL},
            ...
            AngleType::AngleType()
            {
                _literal = kAngleTypeEnumString[0];
                _angleTypeEnum = GetFromLiteral(_literal);
            }

            AngleType::AngleType(AngleTypeEnum& angleType)
            {
                _literal = kAngleTypeEnumString[angleType+1];
                _angleTypeEnum = GetFromLiteral(_literal);
            }

            AngleType::AngleType(const AngleTypeEnum angleType)
            {
                _literal = kAngleTypeEnumString[angleType+1];
                _angleTypeEnum = GetFromLiteral(_literal);
            }

            AngleType::AngleType(std::string& literal_):_literal(literal_) 
            {
                _angleTypeEnum = GetFromLiteral(_literal);
            }

            bool AngleType::IsDeprecated(const AngleTypeEnum angleType)
            { 
            	return false;
            }
			
			std::string AngleType::GetDeprecatedVersion(const AngleTypeEnum angleType)
			{
            	return "";
			}

			std::string AngleType::GetDeprecatedComment(const AngleTypeEnum angleType)
			{
            	return "";
		}
            
            std::string AngleType::GetLiteral() const 
            { 
            	return _literal;
            }

            AngleType& AngleType::operator= (const AngleTypeEnum &rhs)
            {
                _angleTypeEnum = rhs;
                _literal = kAngleTypeEnumString[_angleTypeEnum+1];
                return *this;
            }

            AngleType& AngleType::operator= (const AngleType &rhs ) 
            { 
                _literal = rhs._literal;
                _angleTypeEnum = rhs._angleTypeEnum;
                return *this;
            }

            bool AngleType::operator== (const AngleTypeEnum &rhs) const
            {
                return _angleTypeEnum == rhs;
            }

            AngleType::AngleTypeEnum AngleType::GetFromLiteral(const std::string literal_)
            {
