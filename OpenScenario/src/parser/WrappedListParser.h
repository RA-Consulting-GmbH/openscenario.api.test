...
namespace RAC_OPENSCENARIO
{
    ...
    */
    ...
    protected:
        ...
        RAC_XML_INDEXER::Position _wrapperTagNameEndPosition;
    ...
    public:
        ...
        void ParseSubElements(std::vector<std::shared_ptr<IndexedElement>>& parentElements, ParserContext& parserContext, T& object) override
        {
            int currentOccurs = 0;
            for (auto && indexedElement : parentElements)
            {
                ...
                else
                {
                    ...
                    {
                        auto parserContextPtr = std::make_shared<ParserContext>(parserContext);
                        auto objectPtr = std::make_shared<T>(object);
                        _innerElementParser->Parse(indexedElement, parserContextPtr, objectPtr);
                        currentOccurs++;
                    }
                    else
                    {
