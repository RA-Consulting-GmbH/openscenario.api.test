...
public class XmlScenarioImportLoader implements IScenarioLoader
{
   ...
   * Constructor
   * 
   * @param innerScenarioLoader scenario loader that is used to get properties
   *          from (Resource locators, filename)
   * @param catalogMessageLogger message logger for catalogs.
   */
  public XmlScenarioImportLoader(XmlScenarioLoader innerScenarioLoader, IParserMessageLogger catalogMessageLogger)
  {
    super();
  ...
  /**
   * The map for the list of parameter assignment that maps parameter names to
   * assigned parameter values.
   * 
   * @param parameterAssignments list of parameter assignments from a catalog
   *          reference
   * @param parserMessageLogger to log errors and warnings
   * @return the table that maps parameter names to parameter values
   */
  private Hashtable<String, String> getMapFromParameterAssignements(List<IParameterAssignment> parameterAssignments,
      IParserMessageLogger parserMessageLogger)
  {
    ...
    if (parameterAssignments != null)
    {
      for (IParameterAssignment parameterAssignment : parameterAssignments)
      {
        ...
        {
          parserMessageLogger
              .logMessage(new FileContentMessage("Parameter '" + parameterName + "assigned multiple times (last wins)",
                  ErrorLevel.WARNING, ((ParameterAssignmentImpl) parameterAssignment).getStartMarker()));
        }
   ...
   */
  private Set<String> getCatalogLocations(IOpenScenario openScenario, IResourceLocator resourceLocator, String filename,
      IParserMessageLogger messageLogger)
  {
    Set<String> result = new HashSet<String>();

    IScenarioDefinition scenarioDefinition = openScenario.getOpenScenarioCategory().getScenarioDefinition();
    if (scenarioDefinition != null)
    {
      ICatalogLocations catalogLocations = scenarioDefinition.getCatalogLocations();
      if (catalogLocations != null)
      {
        IControllerCatalogLocation controllerCatalogLocation = catalogLocations.getControllerCatalog();
        ...
        IMiscObjectCatalogLocation miscObjectCatalogLocation = catalogLocations.getMiscObjectCatalog();
        ...
        ITrajectoryCatalogLocation trajectoryCatalogLocation = catalogLocations.getTrajectoryCatalog();
        ...
        IEnvironmentCatalogLocation environmentCatalogLocation = catalogLocations.getEnvironmentCatalog();
        ...
        IPedestrianCatalogLocation pedestrianCatalogLocation = catalogLocations.getPedestrianCatalog();
   ...
   */
  private void addPath(IResourceLocator resourceLocator, String filename, IParserMessageLogger messageLogger, Set<String> result,
      ...
      if (path != null && !path.isEmpty())
      {
        ...
        else
        {
          ...
          messageLogger.logMessage(
              new FileContentMessage("Cannot resolve catalog directory '" + filename + path + "' (" + symbolicDirname + ")",
      ...
      throws ScenarioLoaderException
  {
    ...
    IOpenScenario openScenario = (IOpenScenario) innerScenarioLoader.load(messageLoggerEnvelope).getAdapter(IOpenScenario.class);
    ...
    if (!messageLoggerEnvelope.hasErrors())
    {
      ...
      Set<String> catalogLocations = getCatalogLocations(openScenario, resourceLocator, filename, messageLoggerEnvelope);
      ...
      {
        List<String> filenames = resourceLocator.getSymbolicFilenamesInSymbolicDir(catalogLocationPath);
        for (String symbolicFilename : filenames)
        {
          XmlScenarioLoaderFactory xmlScenarioLoaderFactory = new XmlScenarioLoaderFactory(symbolicFilename);
      ...
      List<ICatalogReference> catalogReferences = ((ICatalogReferenceProvider) openScenario
          .getAdapter(ICatalogReferenceProvider.class)).getCatalogReferences();
      ...
      for (ICatalogReference catalogReference : catalogReferences)
      {
        ...
        else
        {
          messageLoggerEnvelope
              .logMessage(new FileContentMessage(
                  "Cannot resolve entry '" + catalogReference.getEntryName() + "' in catalog '"
                      + catalogReference.getCatalogName() + "'",
                  ErrorLevel.ERROR, ((CatalogReferenceImpl) catalogReference).getStartMarker()));
        }
      }
