...
public class ParameterResolver
{
   ...
   * @param logUnresolvableParameter a flag whether unresolvable parameters
   *          should be logged.
   */
  private void resolveInternal(IParserMessageLogger logger, IParameterizedObject parameterizedObject,
      boolean logUnresolvableParameter)
  {
    ...
    for (String attributeKey : attributeKeys)
    {
      ...
      String value = findValue(parameterizedObject.getTypeFromAttributeName(attributeKey), parameterName);
      ...
      else
      {
        ...
        {
          logger.logMessage(new FileContentMessage("Cannot resolve parameter '" + parameterName + "'", ErrorLevel.ERROR,
   ...
   * @param baseImpl instance of BaseImpl for which the parameters should be
   *          resolved.
   * @param injectedParameters name value pairs injected from outside
   * @param logUnresolvableParameter a flag whether unresolvable parameters
   *          should be logged.
   */
  public void resolve(IParserMessageLogger logger, BaseImpl baseImpl, Map<String, String> injectedParameters,
      boolean logUnresolvableParameter)
  {
    ...
    if (hasParameterDefinitions)
    {
      ...
      if (injectedParameters != null && !injectedParameters.isEmpty() && baseImpl instanceof IScenarioDefinition)
      {
        // override parameter values with injected parameters
        overrideGlobalParametersWithInjectedParameters(parameterDefinitions, logger, injectedParameters,
            (IScenarioDefinition) baseImpl);
      }
    ...
    if (children != null)
    {
      ...
      {
        resolve(logger, child, baseImpl instanceof IScenarioDefinition ? null : injectedParameters, logUnresolvableParameter);
      }
    }
   ...
   * @param scenarioDefinition the scenario definition with the global parameter
   *          declarations
   */
  private void overrideGlobalParametersWithInjectedParameters(List<ParameterValue> parameterDefinitions,
      IParserMessageLogger logger, Map<String, String> injectedParameters, IScenarioDefinition scenarioDefinition)
  {
    ...
    if (parameterDeclarations != null)
    {
      for (IParameterDeclaration parameterDeclaration : parameterDeclarations)
      {
        ...
        if (injectedValue != null)
        {
          ...
          {
            logger.logMessage(
                new FileContentMessage("Injected parameter '" + name + "': " + e.getMessage() + " Injected parameter is ignored.",
    ...
    for (String name : notUsedInjectedParameters)
    {
      logger.logMessage(new FileContentMessage(
          "Injected parameter '" + name + "' must be declared as a global parameter. Injected parameter is ignored.",
          ErrorLevel.WARNING, textmarker));
    }
  ...
  /**
   * Resolve all parameters with parameter assignments, instead of parameter
   * definitions
   * 
   * @param logger to log messages
   * @param catalogElement a catalogElement the parameters will be resolved.
   * @param parameterAssignments the parameter assignments that provide the
   *          parameter names and values
   */
  public void resolveWithParameterAssignments(IParserMessageLogger logger, ICatalogElement catalogElement,
