...
import de.rac.openscenario.v1_0.impl.SimulationTimeConditionImpl;
...
*/
public class SimulationTimeConditionXmlParser extends XmlComplexTypeParser<SimulationTimeConditionImpl> {
	...
	@Override
	public void parseElement(IndexedElement indexedElement, ParserContext parserContext,SimulationTimeConditionImpl object) {
		messageLogger.logMessage(new FileContentMessage("Start Parsing SimulationTimeCondition", ErrorLevel.DEBUG, new Textmarker(indexedElement.getStartElementLocation().getLine(), indexedElement.getStartElementLocation().getColumn(), filename))); 
		super.parseElement(indexedElement,  parserContext, object);
		messageLogger.logMessage(new FileContentMessage("End Parsing SimulationTimeCondition", ErrorLevel.DEBUG, new Textmarker(indexedElement.getEndElementLocation().getLine(), indexedElement.getEndElementLocation().getColumn(), filename))); 
	
	}			
	
	@Override
	protected  Map<String, IAttributeParser<SimulationTimeConditionImpl>> getAttributeNameToAttributeParserMap()
	{
		Map<String, IAttributeParser<SimulationTimeConditionImpl>> result  = new Hashtable<String, IAttributeParser<SimulationTimeConditionImpl>>();
		result.put(OscConstants.ATTRIBUTE__VALUE, new IAttributeParser<SimulationTimeConditionImpl>() {
			@Override
			public void parse(Position startPosition, Position endPosition, String attributeName, String attributeValue, SimulationTimeConditionImpl object)
			{
		...
		result.put(OscConstants.ATTRIBUTE__RULE, new IAttributeParser<SimulationTimeConditionImpl>() {
			@Override
			public void parse(Position startPosition, Position endPosition, String attributeName, String attributeValue, SimulationTimeConditionImpl object)
			{
	...
	private class SubElementParser extends XmlSequenceParser<SimulationTimeConditionImpl>{
		...
		{
			List<IElementParser<SimulationTimeConditionImpl>> result = new ArrayList<IElementParser<SimulationTimeConditionImpl>>();
			return result;
