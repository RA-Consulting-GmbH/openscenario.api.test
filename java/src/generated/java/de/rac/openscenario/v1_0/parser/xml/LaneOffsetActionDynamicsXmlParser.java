...
*/
public class LaneOffsetActionDynamicsXmlParser extends XmlComplexTypeParser<LaneOffsetActionDynamicsImpl> {
	...
	@Override
	public void parseElement(IndexedElement indexedElement, ParserContext parserContext,LaneOffsetActionDynamicsImpl object) {
		messageLogger.logMessage(new FileContentMessage("Start Parsing LaneOffsetActionDynamics", ErrorLevel.DEBUG, new Textmarker(indexedElement.getStartElementLocation().getLine(), indexedElement.getStartElementLocation().getColumn(), filename))); 
		super.parseElement(indexedElement,  parserContext, object);
		messageLogger.logMessage(new FileContentMessage("End Parsing LaneOffsetActionDynamics", ErrorLevel.DEBUG, new Textmarker(indexedElement.getEndElementLocation().getLine(), indexedElement.getEndElementLocation().getColumn(), filename))); 
	
	}			
	
	@Override
	protected  Map<String, IAttributeParser<LaneOffsetActionDynamicsImpl>> getAttributeNameToAttributeParserMap()
	{
		Map<String, IAttributeParser<LaneOffsetActionDynamicsImpl>> result  = new Hashtable<String, IAttributeParser<LaneOffsetActionDynamicsImpl>>();
		result.put(OscConstants.ATTRIBUTE__MAX_LATERAL_ACC, new IAttributeParser<LaneOffsetActionDynamicsImpl>() {
			@Override
			public void parse(Position startPosition, Position endPosition, String attributeName, String attributeValue, LaneOffsetActionDynamicsImpl object)
			{
				...
				{
					object.setAttributeParameter(OscConstants.ATTRIBUTE__MAX_LATERAL_ACC, stripDollarSign(attributeValue), startMarker); 
				}else
				{
					// Parse value
					// Simple type
		...
		result.put(OscConstants.ATTRIBUTE__DYNAMICS_SHAPE, new IAttributeParser<LaneOffsetActionDynamicsImpl>() {
			@Override
			public void parse(Position startPosition, Position endPosition, String attributeName, String attributeValue, LaneOffsetActionDynamicsImpl object)
			{
	...
	private class SubElementParser extends XmlSequenceParser<LaneOffsetActionDynamicsImpl>{
		...
		{
			List<IElementParser<LaneOffsetActionDynamicsImpl>> result = new ArrayList<IElementParser<LaneOffsetActionDynamicsImpl>>();
			return result;
