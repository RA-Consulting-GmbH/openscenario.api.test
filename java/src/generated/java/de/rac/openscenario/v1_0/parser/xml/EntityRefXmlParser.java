...
public class EntityRefXmlParser extends XmlComplexTypeParser<EntityRefImpl> {
	...
	@Override
	public void parseElement(IndexedElement indexedElement, ParserContext parserContext,EntityRefImpl object) {
		messageLogger.logMessage(new FileContentMessage("Start Parsing EntityRef", ErrorLevel.DEBUG, new Textmarker(indexedElement.getStartElementLocation().getLine(), indexedElement.getStartElementLocation().getColumn(), filename))); 
		super.parseElement(indexedElement,  parserContext, object);
		messageLogger.logMessage(new FileContentMessage("End Parsing EntityRef", ErrorLevel.DEBUG, new Textmarker(indexedElement.getEndElementLocation().getLine(), indexedElement.getEndElementLocation().getColumn(), filename))); 
	...
	{
		Map<String, IAttributeParser<EntityRefImpl>> result  = new Hashtable<String, IAttributeParser<EntityRefImpl>>();
		result.put(OscConstants.ATTRIBUTE__ENTITY_REF, new IAttributeParser<EntityRefImpl>() {
			@Override
			public void parse(Position startPosition, Position endPosition, String attributeName, String attributeValue, EntityRefImpl object)
			{
