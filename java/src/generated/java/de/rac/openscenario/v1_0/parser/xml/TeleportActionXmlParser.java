...
public class TeleportActionXmlParser extends XmlComplexTypeParser<TeleportActionImpl> {
	...
	@Override
	public void parseElement(IndexedElement indexedElement, ParserContext parserContext,TeleportActionImpl object) {
		messageLogger.logMessage(new FileContentMessage("Start Parsing TeleportAction", ErrorLevel.DEBUG, new Textmarker(indexedElement.getStartElementLocation().getLine(), indexedElement.getStartElementLocation().getColumn(), filename))); 
		super.parseElement(indexedElement,  parserContext, object);
		messageLogger.logMessage(new FileContentMessage("End Parsing TeleportAction", ErrorLevel.DEBUG, new Textmarker(indexedElement.getEndElementLocation().getLine(), indexedElement.getEndElementLocation().getColumn(), filename))); 
	
	}			
	
	@Override
	protected  Map<String, IAttributeParser<TeleportActionImpl>> getAttributeNameToAttributeParserMap()
	{
		Map<String, IAttributeParser<TeleportActionImpl>> result  = new Hashtable<String, IAttributeParser<TeleportActionImpl>>();
		return result;
	}
	...
	private class SubElementParser extends XmlSequenceParser<TeleportActionImpl>{
		...
		{
			List<IElementParser<TeleportActionImpl>> result = new ArrayList<IElementParser<TeleportActionImpl>>();
	...
	private class SubElementPositionParser implements IElementParser<TeleportActionImpl> {
		...
		@Override
		public void parse(IndexedElement indexedElement, ParserContext parserContext,TeleportActionImpl object)
		{
