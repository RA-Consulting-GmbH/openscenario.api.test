...
public class ControlPointXmlParser extends XmlComplexTypeParser<ControlPointImpl> {
	...
	@Override
	public void parseElement(IndexedElement indexedElement, ParserContext parserContext,ControlPointImpl object) {
		messageLogger.logMessage(new FileContentMessage("Start Parsing ControlPoint", ErrorLevel.DEBUG, new Textmarker(indexedElement.getStartElementLocation().getLine(), indexedElement.getStartElementLocation().getColumn(), filename))); 
		super.parseElement(indexedElement,  parserContext, object);
		messageLogger.logMessage(new FileContentMessage("End Parsing ControlPoint", ErrorLevel.DEBUG, new Textmarker(indexedElement.getEndElementLocation().getLine(), indexedElement.getEndElementLocation().getColumn(), filename))); 
	...
	{
		Map<String, IAttributeParser<ControlPointImpl>> result  = new Hashtable<String, IAttributeParser<ControlPointImpl>>();
		result.put(OscConstants.ATTRIBUTE__TIME, new IAttributeParser<ControlPointImpl>() {
			@Override
			public void parse(Position startPosition, Position endPosition, String attributeName, String attributeValue, ControlPointImpl object)
			{
		...
		result.put(OscConstants.ATTRIBUTE__WEIGHT, new IAttributeParser<ControlPointImpl>() {
			@Override
			public void parse(Position startPosition, Position endPosition, String attributeName, String attributeValue, ControlPointImpl object)
			{
	...
	private class SubElementParser extends XmlSequenceParser<ControlPointImpl>{
		...
		{
			List<IElementParser<ControlPointImpl>> result = new ArrayList<IElementParser<ControlPointImpl>>();
	...
	private class SubElementPositionParser implements IElementParser<ControlPointImpl> {
		...
		@Override
		public void parse(IndexedElement indexedElement, ParserContext parserContext,ControlPointImpl object)
		{
