...
public class KnotXmlParser extends XmlComplexTypeParser<KnotImpl> {
	...
	@Override
	public void parseElement(IndexedElement indexedElement, ParserContext parserContext,KnotImpl object) {
		messageLogger.logMessage(new FileContentMessage("Start Parsing Knot", ErrorLevel.DEBUG, new Textmarker(indexedElement.getStartElementLocation().getLine(), indexedElement.getStartElementLocation().getColumn(), filename))); 
		super.parseElement(indexedElement,  parserContext, object);
		messageLogger.logMessage(new FileContentMessage("End Parsing Knot", ErrorLevel.DEBUG, new Textmarker(indexedElement.getEndElementLocation().getLine(), indexedElement.getEndElementLocation().getColumn(), filename))); 
	...
	{
		Map<String, IAttributeParser<KnotImpl>> result  = new Hashtable<String, IAttributeParser<KnotImpl>>();
		result.put(OscConstants.ATTRIBUTE__VALUE, new IAttributeParser<KnotImpl>() {
			@Override
			public void parse(Position startPosition, Position endPosition, String attributeName, String attributeValue, KnotImpl object)
			{
