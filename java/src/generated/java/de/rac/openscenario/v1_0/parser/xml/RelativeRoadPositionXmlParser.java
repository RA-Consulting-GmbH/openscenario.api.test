...
public class RelativeRoadPositionXmlParser extends XmlComplexTypeParser<RelativeRoadPositionImpl> {
	...
	@Override
	public void parseElement(IndexedElement indexedElement, ParserContext parserContext,RelativeRoadPositionImpl object) {
		messageLogger.logMessage(new FileContentMessage("Start Parsing RelativeRoadPosition", ErrorLevel.DEBUG, new Textmarker(indexedElement.getStartElementLocation().getLine(), indexedElement.getStartElementLocation().getColumn(), filename))); 
		super.parseElement(indexedElement,  parserContext, object);
		messageLogger.logMessage(new FileContentMessage("End Parsing RelativeRoadPosition", ErrorLevel.DEBUG, new Textmarker(indexedElement.getEndElementLocation().getLine(), indexedElement.getEndElementLocation().getColumn(), filename))); 
	
	}			
	
	@Override
	protected  Map<String, IAttributeParser<RelativeRoadPositionImpl>> getAttributeNameToAttributeParserMap()
	{
		Map<String, IAttributeParser<RelativeRoadPositionImpl>> result  = new Hashtable<String, IAttributeParser<RelativeRoadPositionImpl>>();
		result.put(OscConstants.ATTRIBUTE__ENTITY_REF, new IAttributeParser<RelativeRoadPositionImpl>() {
			@Override
			public void parse(Position startPosition, Position endPosition, String attributeName, String attributeValue, RelativeRoadPositionImpl object)
			{
		...
		result.put(OscConstants.ATTRIBUTE__DS, new IAttributeParser<RelativeRoadPositionImpl>() {
			@Override
			public void parse(Position startPosition, Position endPosition, String attributeName, String attributeValue, RelativeRoadPositionImpl object)
			{
		...
		result.put(OscConstants.ATTRIBUTE__DT, new IAttributeParser<RelativeRoadPositionImpl>() {
			@Override
			public void parse(Position startPosition, Position endPosition, String attributeName, String attributeValue, RelativeRoadPositionImpl object)
			{
				...
				if (isParametrized(attributeValue))
				{
					object.setAttributeParameter(OscConstants.ATTRIBUTE__DT, stripDollarSign(attributeValue), startMarker); 
				}else
				{
					// Parse value
					// Simple type
	...
	private class SubElementParser extends XmlAllParser<RelativeRoadPositionImpl>{
		...
		{
			List<IElementParser<RelativeRoadPositionImpl>> result = new ArrayList<IElementParser<RelativeRoadPositionImpl>>();
	...
	private class SubElementOrientationParser implements IElementParser<RelativeRoadPositionImpl> {
		...
		@Override
		public void parse(IndexedElement indexedElement, ParserContext parserContext,RelativeRoadPositionImpl object)
		{
