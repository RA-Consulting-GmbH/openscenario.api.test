...
*/
public class RelativeSpeedConditionXmlParser extends XmlComplexTypeParser<RelativeSpeedConditionImpl> {
	...
	@Override
	public void parseElement(IndexedElement indexedElement, ParserContext parserContext,RelativeSpeedConditionImpl object) {
		messageLogger.logMessage(new FileContentMessage("Start Parsing RelativeSpeedCondition", ErrorLevel.DEBUG, new Textmarker(indexedElement.getStartElementLocation().getLine(), indexedElement.getStartElementLocation().getColumn(), filename))); 
		super.parseElement(indexedElement,  parserContext, object);
		messageLogger.logMessage(new FileContentMessage("End Parsing RelativeSpeedCondition", ErrorLevel.DEBUG, new Textmarker(indexedElement.getEndElementLocation().getLine(), indexedElement.getEndElementLocation().getColumn(), filename))); 
	
	}			
	
	@Override
	protected  Map<String, IAttributeParser<RelativeSpeedConditionImpl>> getAttributeNameToAttributeParserMap()
	{
		Map<String, IAttributeParser<RelativeSpeedConditionImpl>> result  = new Hashtable<String, IAttributeParser<RelativeSpeedConditionImpl>>();
		result.put(OscConstants.ATTRIBUTE__ENTITY_REF, new IAttributeParser<RelativeSpeedConditionImpl>() {
			@Override
			public void parse(Position startPosition, Position endPosition, String attributeName, String attributeValue, RelativeSpeedConditionImpl object)
			{
		...
		result.put(OscConstants.ATTRIBUTE__VALUE, new IAttributeParser<RelativeSpeedConditionImpl>() {
			@Override
			public void parse(Position startPosition, Position endPosition, String attributeName, String attributeValue, RelativeSpeedConditionImpl object)
			{
		...
		result.put(OscConstants.ATTRIBUTE__RULE, new IAttributeParser<RelativeSpeedConditionImpl>() {
			@Override
			public void parse(Position startPosition, Position endPosition, String attributeName, String attributeValue, RelativeSpeedConditionImpl object)
			{
	...
	private class SubElementParser extends XmlSequenceParser<RelativeSpeedConditionImpl>{
		...
		{
			List<IElementParser<RelativeSpeedConditionImpl>> result = new ArrayList<IElementParser<RelativeSpeedConditionImpl>>();
			return result;
