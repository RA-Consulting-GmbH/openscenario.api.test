 ...
 * cardinality checker rule class for the type IFileHeader according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class FileHeaderCardinalityCheckerRule extends CardinalityCheckerRule<IFileHeader> {

  /** Default constructor */
  public FileHeaderCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IFileHeader object) {
    ...
    if (object.getRevMajor() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__REV_MAJOR, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getRevMinor() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__REV_MINOR, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getDate() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__DATE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getDescription() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__DESCRIPTION, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getAuthor() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__AUTHOR, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
