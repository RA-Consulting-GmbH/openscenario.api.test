 ...
 * cardinality checker rule class for the type ISpeedAction according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class SpeedActionCardinalityCheckerRule extends CardinalityCheckerRule<ISpeedAction> {

  /** Default constructor */
  public SpeedActionCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, ISpeedAction object) {
  ...
  private List<CardinalityViolation> getAllViolations(ISpeedAction object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getSpeedActionDynamics() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__SPEED_ACTION_DYNAMICS, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getSpeedActionTarget() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__SPEED_ACTION_TARGET, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, ISpeedAction object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
