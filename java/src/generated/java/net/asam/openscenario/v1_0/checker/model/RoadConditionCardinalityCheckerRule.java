 ...
 * cardinality checker rule class for the type IRoadCondition according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class RoadConditionCardinalityCheckerRule extends CardinalityCheckerRule<IRoadCondition> {

  /** Default constructor */
  public RoadConditionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IRoadCondition object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getFrictionScaleFactor() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__FRICTION_SCALE_FACTOR, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
