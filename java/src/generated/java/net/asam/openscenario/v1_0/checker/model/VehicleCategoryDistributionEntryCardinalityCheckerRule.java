 ...
 * cardinality checker rule class for the type IVehicleCategoryDistributionEntry according to
 * OpenSCENARIO standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class VehicleCategoryDistributionEntryCardinalityCheckerRule
    extends CardinalityCheckerRule<IVehicleCategoryDistributionEntry> {

  /** Default constructor */
  public VehicleCategoryDistributionEntryCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IVehicleCategoryDistributionEntry object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getCategory() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__CATEGORY, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getWeight() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__WEIGHT, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
