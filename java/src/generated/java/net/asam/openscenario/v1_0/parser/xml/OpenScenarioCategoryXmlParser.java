...
public class OpenScenarioCategoryXmlParser extends XmlGroupParser<OpenScenarioCategoryImpl> {
	...
	@Override
	public void parseElement(IndexedElement indexedElement, ParserContext parserContext,OpenScenarioCategoryImpl object) {
		messageLogger.logMessage(new FileContentMessage("Start Parsing OpenScenarioCategory", ErrorLevel.DEBUG, new Textmarker(indexedElement.getStartElementLocation().getLine(), indexedElement.getStartElementLocation().getColumn(), filename))); 
		super.parseElement(indexedElement,  parserContext, object);
		messageLogger.logMessage(new FileContentMessage("End Parsing OpenScenarioCategory", ErrorLevel.DEBUG, new Textmarker(indexedElement.getEndElementLocation().getLine(), indexedElement.getEndElementLocation().getColumn(), filename))); 
	...
	private class SubElementParser extends XmlChoiceParser<OpenScenarioCategoryImpl>{
		...
		{
			List<IElementParser<OpenScenarioCategoryImpl>> result = new ArrayList<IElementParser<OpenScenarioCategoryImpl>>();
	 ...
	 */
	private class SubElementScenarioDefinitionParser implements IElementParser<OpenScenarioCategoryImpl> {
		...
		@Override
		public void parse(IndexedElement indexedElement, ParserContext parserContext,OpenScenarioCategoryImpl object)
		{
	 ...
	 */
	private class SubElementCatalogDefinitionParser implements IElementParser<OpenScenarioCategoryImpl> {
