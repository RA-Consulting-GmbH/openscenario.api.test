 ...
 * cardinality checker rule class for the type IClothoid according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class ClothoidCardinalityCheckerRule extends CardinalityCheckerRule<IClothoid> {

  /** Default constructor */
  public ClothoidCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IClothoid object) {
    ...
    if (object.getCurvature() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__CURVATURE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getCurvatureDot() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__CURVATURE_DOT, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getLength() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__LENGTH, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
