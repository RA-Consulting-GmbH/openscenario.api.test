 ...
 * cardinality checker rule class for the type IPositionInRoadCoordinates according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class PositionInRoadCoordinatesCardinalityCheckerRule
    extends CardinalityCheckerRule<IPositionInRoadCoordinates> {

  /** Default constructor */
  public PositionInRoadCoordinatesCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IPositionInRoadCoordinates object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getPathS() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__PATH_S, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getT() == null) {
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, IPositionInRoadCoordinates object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
