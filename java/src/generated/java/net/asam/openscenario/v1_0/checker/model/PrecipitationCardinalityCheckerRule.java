 ...
 * cardinality checker rule class for the type IPrecipitation according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class PrecipitationCardinalityCheckerRule extends CardinalityCheckerRule<IPrecipitation> {

  /** Default constructor */
  public PrecipitationCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IPrecipitation object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getPrecipitationType() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__PRECIPITATION_TYPE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getIntensity() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__INTENSITY, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
