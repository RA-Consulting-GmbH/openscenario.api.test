 ...
 * cardinality checker rule class for the type IRelativeRoadPosition according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class RelativeRoadPositionCardinalityCheckerRule
    extends CardinalityCheckerRule<IRelativeRoadPosition> {

  /** Default constructor */
  public RelativeRoadPositionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IRelativeRoadPosition object) {
    ...
    // Check violation
    // Check required
    if (object.getDt() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__DT, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, IRelativeRoadPosition object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
