 ...
 * cardinality checker rule class for the type ITrafficSignalControllerCondition according to
 * OpenSCENARIO standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class TrafficSignalControllerConditionCardinalityCheckerRule
    extends CardinalityCheckerRule<ITrafficSignalControllerCondition> {

  /** Default constructor */
  public TrafficSignalControllerConditionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(ITrafficSignalControllerCondition object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getTrafficSignalControllerRef() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__TRAFFIC_SIGNAL_CONTROLLER_REF, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getPhase() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__PHASE, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, ITrafficSignalControllerCondition object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
