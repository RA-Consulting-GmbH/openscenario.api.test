 ...
 * cardinality checker rule class for the type IDimensions according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class DimensionsCardinalityCheckerRule extends CardinalityCheckerRule<IDimensions> {

  /** Default constructor */
  public DimensionsCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IDimensions object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getWidth() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__WIDTH, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    ...
    if (object.getHeight() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__HEIGHT, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
