 ...
 * union checker rule class for the type IOpenScenarioCategory according to OpenSCENARIO standard
 * all xor elements are checked
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class OpenScenarioCategoryUnionCheckerRule extends UnionCheckerRule<IOpenScenarioCategory> {

  /** Default constructor */
  public OpenScenarioCategoryUnionCheckerRule() {
    super();
  }
  ...
  private List<String> getNotNullChildren(IOpenScenarioCategory object) {
    List<String> propertyNamesNotNull = new ArrayList<>();
    if (object.getScenarioDefinition() != null) {
      propertyNamesNotNull.add(OscConstants.ELEMENT__SCENARIO_DEFINITION);
    }

    if (object.getCatalogDefinition() != null) {
      propertyNamesNotNull.add(OscConstants.ELEMENT__CATALOG_DEFINITION);
    }
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, IOpenScenarioCategory object) {
    List<String> propertyNamesNotNull = getNotNullChildren(object);
    if (propertyNamesNotNull.size() > 1) {
      messageLogger.logMessage(
    ...
    if (propertyNamesNotNull.size() == 0) {
      messageLogger.logMessage(
          new TreeContentMessage(
              getTooFewMessage(propertyNamesNotNull),
              ErrorLevel.ERROR,
              new PropertyTreeContext(object, getAllChildren())));
    }
  }
}
