 ...
 * cardinality checker rule class for the type IByObjectType according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class ByObjectTypeCardinalityCheckerRule extends CardinalityCheckerRule<IByObjectType> {

  /** Default constructor */
  public ByObjectTypeCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IByObjectType object) {
  ...
  private List<CardinalityViolation> getAllViolations(IByObjectType object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getType() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__TYPE, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IByObjectType object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
