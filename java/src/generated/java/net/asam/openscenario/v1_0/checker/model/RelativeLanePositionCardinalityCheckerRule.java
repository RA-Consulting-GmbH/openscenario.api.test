 ...
 * cardinality checker rule class for the type IRelativeLanePosition according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class RelativeLanePositionCardinalityCheckerRule
    extends CardinalityCheckerRule<IRelativeLanePosition> {

  /** Default constructor */
  public RelativeLanePositionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IRelativeLanePosition object) {
    ...
    // Check violation
    // Check required
    if (object.getDLane() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__D_LANE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getDs() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__DS, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, IRelativeLanePosition object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
