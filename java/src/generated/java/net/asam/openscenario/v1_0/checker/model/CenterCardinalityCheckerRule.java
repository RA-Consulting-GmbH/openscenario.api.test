 ...
 * cardinality checker rule class for the type ICenter according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class CenterCardinalityCheckerRule extends CardinalityCheckerRule<ICenter> {

  /** Default constructor */
  public CenterCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, ICenter object) {
  ...
  private List<CardinalityViolation> getAllViolations(ICenter object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getX() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__X, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getY() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__Y, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getZ() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__Z, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, ICenter object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
