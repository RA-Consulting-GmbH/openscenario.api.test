 ...
 * cardinality checker rule class for the type IRelativeDistanceCondition according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class RelativeDistanceConditionCardinalityCheckerRule
    extends CardinalityCheckerRule<IRelativeDistanceCondition> {

  /** Default constructor */
  public RelativeDistanceConditionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IRelativeDistanceCondition object) {
    ...
    // Check violation
    // Check required
    if (object.getRelativeDistanceType() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__RELATIVE_DISTANCE_TYPE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, IRelativeDistanceCondition object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
