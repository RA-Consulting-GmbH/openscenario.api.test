 ...
 * cardinality checker rule class for the type IControllerAction according to OpenSCENARIO standard
 * all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class ControllerActionCardinalityCheckerRule
    extends CardinalityCheckerRule<IControllerAction> {

  /** Default constructor */
  public ControllerActionCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IControllerAction object) {
  ...
  private List<CardinalityViolation> getAllViolations(IControllerAction object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getAssignControllerAction() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__ASSIGN_CONTROLLER_ACTION, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getOverrideControllerValueAction() == null) {
      violations.add(
          new CardinalityViolation(
  ...
  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IControllerAction object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
