 ...
 * cardinality checker rule class for the type ITrafficDefinition according to OpenSCENARIO standard
 * all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class TrafficDefinitionCardinalityCheckerRule
    extends CardinalityCheckerRule<ITrafficDefinition> {

  /** Default constructor */
  public TrafficDefinitionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(ITrafficDefinition object) {
    ...
    // Check violation
    // Check required
    if (object.getVehicleCategoryDistribution() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__VEHICLE_CATEGORY_DISTRIBUTION, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getControllerDistribution() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__CONTROLLER_DISTRIBUTION, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, ITrafficDefinition object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
