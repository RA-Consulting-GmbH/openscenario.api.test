 ...
 * cardinality checker rule class for the type ITrafficSwarmAction according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class TrafficSwarmActionCardinalityCheckerRule
    extends CardinalityCheckerRule<ITrafficSwarmAction> {

  /** Default constructor */
  public TrafficSwarmActionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(ITrafficSwarmAction object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getSemiMajorAxis() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__SEMI_MAJOR_AXIS, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getSemiMinorAxis() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__SEMI_MINOR_AXIS, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getInnerRadius() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__INNER_RADIUS, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getOffset() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__OFFSET, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getNumberOfVehicles() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__NUMBER_OF_VEHICLES, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getCentralObject() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__CENTRAL_OBJECT, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getTrafficDefinition() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__TRAFFIC_DEFINITION, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
