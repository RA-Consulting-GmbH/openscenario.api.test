 ...
 * cardinality checker rule class for the type IOverrideParkingBrakeAction according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class OverrideParkingBrakeActionCardinalityCheckerRule
    extends CardinalityCheckerRule<IOverrideParkingBrakeAction> {

  /** Default constructor */
  public OverrideParkingBrakeActionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IOverrideParkingBrakeAction object) {
    ...
    if (object.getActive() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__ACTIVE, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, IOverrideParkingBrakeAction object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
