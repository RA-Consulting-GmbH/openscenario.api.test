 ...
 * cardinality checker rule class for the type IDirectory according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class DirectoryCardinalityCheckerRule extends CardinalityCheckerRule<IDirectory> {

  /** Default constructor */
  public DirectoryCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IDirectory object) {
  ...
  private List<CardinalityViolation> getAllViolations(IDirectory object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getPath() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__PATH, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IDirectory object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
