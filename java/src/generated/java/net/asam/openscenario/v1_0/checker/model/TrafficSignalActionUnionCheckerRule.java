 ...
 * union checker rule class for the type ITrafficSignalAction according to OpenSCENARIO standard all
 * xor elements are checked
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class TrafficSignalActionUnionCheckerRule extends UnionCheckerRule<ITrafficSignalAction> {

  /** Default constructor */
  public TrafficSignalActionUnionCheckerRule() {
    super();
  }
  ...
  private List<String> getNotNullChildren(ITrafficSignalAction object) {
    List<String> propertyNamesNotNull = new ArrayList<>();
    if (object.getTrafficSignalControllerAction() != null) {
      propertyNamesNotNull.add(OscConstants.ELEMENT__TRAFFIC_SIGNAL_CONTROLLER_ACTION);
    }

    if (object.getTrafficSignalStateAction() != null) {
      propertyNamesNotNull.add(OscConstants.ELEMENT__TRAFFIC_SIGNAL_STATE_ACTION);
    }
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, ITrafficSignalAction object) {
    List<String> propertyNamesNotNull = getNotNullChildren(object);
    if (propertyNamesNotNull.size() > 1) {
      messageLogger.logMessage(
