 ...
 * cardinality checker rule class for the type IOverrideControllerValueAction according to
 * OpenSCENARIO standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class OverrideControllerValueActionCardinalityCheckerRule
    extends CardinalityCheckerRule<IOverrideControllerValueAction> {

  /** Default constructor */
  public OverrideControllerValueActionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IOverrideControllerValueAction object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getThrottle() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__THROTTLE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getBrake() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__BRAKE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getClutch() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__CLUTCH, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getParkingBrake() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__PARKING_BRAKE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getSteeringWheel() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__STEERING_WHEEL, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getGear() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__GEAR, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, IOverrideControllerValueAction object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
