 ...
 * cardinality checker rule class for the type ICatalogReference according to OpenSCENARIO standard
 * all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class CatalogReferenceCardinalityCheckerRule
    extends CardinalityCheckerRule<ICatalogReference> {

  /** Default constructor */
  public CatalogReferenceCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, ICatalogReference object) {
  ...
  private List<CardinalityViolation> getAllViolations(ICatalogReference object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getCatalogName() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__CATALOG_NAME, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getEntryName() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__ENTRY_NAME, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, ICatalogReference object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
