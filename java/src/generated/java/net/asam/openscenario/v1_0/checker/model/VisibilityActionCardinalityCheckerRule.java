 ...
 * cardinality checker rule class for the type IVisibilityAction according to OpenSCENARIO standard
 * all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class VisibilityActionCardinalityCheckerRule
    extends CardinalityCheckerRule<IVisibilityAction> {

  /** Default constructor */
  public VisibilityActionCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IVisibilityAction object) {
  ...
  private List<CardinalityViolation> getAllViolations(IVisibilityAction object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getGraphics() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__GRAPHICS, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getTraffic() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__TRAFFIC, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getSensors() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__SENSORS, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IVisibilityAction object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
