 ...
 * cardinality checker rule class for the type IAxles according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class AxlesCardinalityCheckerRule extends CardinalityCheckerRule<IAxles> {

  /** Default constructor */
  public AxlesCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IAxles object) {
  ...
  private List<CardinalityViolation> getAllViolations(IAxles object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getFrontAxle() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__FRONT_AXLE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getRearAxle() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__REAR_AXLE, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IAxles object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
