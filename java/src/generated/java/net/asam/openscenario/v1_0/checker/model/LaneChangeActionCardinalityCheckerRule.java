 ...
 * cardinality checker rule class for the type ILaneChangeAction according to OpenSCENARIO standard
 * all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class LaneChangeActionCardinalityCheckerRule
    extends CardinalityCheckerRule<ILaneChangeAction> {

  /** Default constructor */
  public LaneChangeActionCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, ILaneChangeAction object) {
  ...
  private List<CardinalityViolation> getAllViolations(ILaneChangeAction object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getLaneChangeActionDynamics() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__LANE_CHANGE_ACTION_DYNAMICS, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getLaneChangeTarget() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__LANE_CHANGE_TARGET, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, ILaneChangeAction object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
