 ...
 * cardinality checker rule class for the type ISynchronizeAction according to OpenSCENARIO standard
 * all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class SynchronizeActionCardinalityCheckerRule
    extends CardinalityCheckerRule<ISynchronizeAction> {

  /** Default constructor */
  public SynchronizeActionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(ISynchronizeAction object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getMasterEntityRef() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__MASTER_ENTITY_REF, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getTargetPositionMaster() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__TARGET_POSITION_MASTER, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getTargetPosition() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__TARGET_POSITION, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, ISynchronizeAction object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
