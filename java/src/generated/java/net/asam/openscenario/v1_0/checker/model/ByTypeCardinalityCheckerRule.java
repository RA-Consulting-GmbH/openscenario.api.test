 ...
 * cardinality checker rule class for the type IByType according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class ByTypeCardinalityCheckerRule extends CardinalityCheckerRule<IByType> {

  /** Default constructor */
  public ByTypeCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IByType object) {
  ...
  private List<CardinalityViolation> getAllViolations(IByType object) {
    ...
    if (object.getObjectType() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__OBJECT_TYPE, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IByType object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
