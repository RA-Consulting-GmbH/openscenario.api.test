 ...
 * cardinality checker rule class for the type IManeuverGroup according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class ManeuverGroupCardinalityCheckerRule extends CardinalityCheckerRule<IManeuverGroup> {

  /** Default constructor */
  public ManeuverGroupCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IManeuverGroup object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getMaximumExecutionCount() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__MAXIMUM_EXECUTION_COUNT, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    ...
    if (object.getActors() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__ACTORS, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
