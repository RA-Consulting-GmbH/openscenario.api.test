 ...
 * cardinality checker rule class for the type IEvent according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class EventCardinalityCheckerRule extends CardinalityCheckerRule<IEvent> {

  /** Default constructor */
  public EventCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IEvent object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getPriority() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__PRIORITY, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    ...
    // Check violation
    int actionsSize = object.getActionsSize();
    // Check too few elements
    if (actionsSize < 1) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__ACTION, 1, actionsSize, ViolationType.TOO_FEW));
    }
    // Check violation
    // Check required
    if (object.getStartTrigger() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__START_TRIGGER, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
