 ...
 * cardinality checker rule class for the type IInfrastructureAction according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class InfrastructureActionCardinalityCheckerRule
    extends CardinalityCheckerRule<IInfrastructureAction> {

  /** Default constructor */
  public InfrastructureActionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IInfrastructureAction object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getTrafficSignalAction() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__TRAFFIC_SIGNAL_ACTION, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, IInfrastructureAction object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
