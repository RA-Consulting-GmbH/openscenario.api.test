 ...
 * cardinality checker rule class for the type IFollowTrajectoryAction according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class FollowTrajectoryActionCardinalityCheckerRule
    extends CardinalityCheckerRule<IFollowTrajectoryAction> {

  /** Default constructor */
  public FollowTrajectoryActionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IFollowTrajectoryAction object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getTimeReference() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__TIME_REFERENCE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getTrajectoryFollowingMode() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__TRAJECTORY_FOLLOWING_MODE, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, IFollowTrajectoryAction object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
