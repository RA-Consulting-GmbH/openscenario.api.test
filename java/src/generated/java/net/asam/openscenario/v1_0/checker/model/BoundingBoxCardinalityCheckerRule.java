 ...
 * cardinality checker rule class for the type IBoundingBox according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class BoundingBoxCardinalityCheckerRule extends CardinalityCheckerRule<IBoundingBox> {

  /** Default constructor */
  public BoundingBoxCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IBoundingBox object) {
  ...
  private List<CardinalityViolation> getAllViolations(IBoundingBox object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getCenter() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__CENTER, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getDimensions() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__DIMENSIONS, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IBoundingBox object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
