...
public class AxleImpl extends BaseImpl implements IAxle, Cloneable{
  ...
  public Double getDoubleProperty(String key) throws KeyNotSupportedException
  {
	...
	if (key.equals(OscConstants.ATTRIBUTE__WHEEL_DIAMETER))
	{
		return getWheelDiameter();
	}else 
	if (key.equals(OscConstants.ATTRIBUTE__TRACK_WIDTH))
	{
		return getTrackWidth();
	}else 
	if (key.equals(OscConstants.ATTRIBUTE__POSITION_X))
	{
		return getPositionX();
	}else 
	if (key.equals(OscConstants.ATTRIBUTE__POSITION_Z))
	{
		return getPositionZ();
	}else 
	{
