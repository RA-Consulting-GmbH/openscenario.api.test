 ...
 *
 * <p>From OpenSCENARIO class model specification: Defines swarm traffic within an elliptical
 * planview around a given central entity. The innerRadius defines the minimum distance between the
 * central vehicle and swarm traffic. The latter is set up in the free space between the circular
 * area defined by innerRadius and the outline of the ellipsis defined by the two semi axis
 * attributes. While vehicles are created in this area, they are removed if they leave it crossing
 * the it ellipsis’ outline. Within the innerRadius, vehicles are neither set up nor removed.
 * Optionally, the swarm area can be longitudinally offset to the central scenario object.
 *
