 ...
 * cardinality checker rule class for the type IWeather according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class WeatherCardinalityCheckerRule extends CardinalityCheckerRule<IWeather> {

  /** Default constructor */
  public WeatherCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IWeather object) {
  ...
  private List<CardinalityViolation> getAllViolations(IWeather object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getCloudState() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__CLOUD_STATE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getSun() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__SUN, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getFog() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__FOG, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getPrecipitation() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__PRECIPITATION, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IWeather object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
