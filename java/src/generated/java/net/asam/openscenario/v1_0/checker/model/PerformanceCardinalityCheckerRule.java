 ...
 * cardinality checker rule class for the type IPerformance according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class PerformanceCardinalityCheckerRule extends CardinalityCheckerRule<IPerformance> {

  /** Default constructor */
  public PerformanceCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IPerformance object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getMaxSpeed() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__MAX_SPEED, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getMaxAcceleration() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__MAX_ACCELERATION, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getMaxDeceleration() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__MAX_DECELERATION, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
