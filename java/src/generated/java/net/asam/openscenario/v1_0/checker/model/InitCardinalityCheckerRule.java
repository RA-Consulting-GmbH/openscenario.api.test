 ...
 * cardinality checker rule class for the type IInit according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class InitCardinalityCheckerRule extends CardinalityCheckerRule<IInit> {

  /** Default constructor */
  public InitCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IInit object) {
  ...
  private List<CardinalityViolation> getAllViolations(IInit object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getActions() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__ACTIONS, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IInit object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
