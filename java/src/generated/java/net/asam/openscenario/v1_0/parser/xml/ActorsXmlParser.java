...
public class ActorsXmlParser extends XmlComplexTypeParser<ActorsImpl> {
	...
	@Override
	public void parseElement(IndexedElement indexedElement, ParserContext parserContext,ActorsImpl object) {
		messageLogger.logMessage(new FileContentMessage("Start Parsing Actors", ErrorLevel.DEBUG, new Textmarker(indexedElement.getStartElementLocation().getLine(), indexedElement.getStartElementLocation().getColumn(), filename))); 
		super.parseElement(indexedElement,  parserContext, object);
		messageLogger.logMessage(new FileContentMessage("End Parsing Actors", ErrorLevel.DEBUG, new Textmarker(indexedElement.getEndElementLocation().getLine(), indexedElement.getEndElementLocation().getColumn(), filename))); 
	...
	{
		Map<String, IAttributeParser<ActorsImpl>> result  = new Hashtable<String, IAttributeParser<ActorsImpl>>();
		result.put(OscConstants.ATTRIBUTE__SELECT_TRIGGERING_ENTITIES, new IAttributeParser<ActorsImpl>() {
			@Override
			public void parse(Position startPosition, Position endPosition, String attributeName, String attributeValue, ActorsImpl object)
			{
				...
				{
					object.setAttributeParameter(OscConstants.ATTRIBUTE__SELECT_TRIGGERING_ENTITIES, stripDollarSign(attributeValue), startMarker); 
				}else
				{
					// Parse value
					// Simple type
				...
				object.putPropertyStartMarker(OscConstants.ATTRIBUTE__SELECT_TRIGGERING_ENTITIES, startMarker);
				object.putPropertyEndMarker(OscConstants.ATTRIBUTE__SELECT_TRIGGERING_ENTITIES, endMarker);
	...
	private class SubElementEntityRefsParser implements IElementParser<ActorsImpl> {
		...
		@Override
		public void parse(IndexedElement indexedElement, ParserContext parserContext,ActorsImpl object)
		{
