 ...
 * cardinality checker rule class for the type IStoryboard according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class StoryboardCardinalityCheckerRule extends CardinalityCheckerRule<IStoryboard> {

  /** Default constructor */
  public StoryboardCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IStoryboard object) {
  ...
  private List<CardinalityViolation> getAllViolations(IStoryboard object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getInit() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__INIT, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    int storiesSize = object.getStoriesSize();
    // Check too few elements
    if (storiesSize < 1) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__STORY, 1, storiesSize, ViolationType.TOO_FEW));
    }
    // Check violation
    // Check required
    if (object.getStopTrigger() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__STOP_TRIGGER, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IStoryboard object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
