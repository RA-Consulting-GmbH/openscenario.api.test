 ...
 * union checker rule class for the type ILongitudinalAction according to OpenSCENARIO standard all
 * xor elements are checked
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class LongitudinalActionUnionCheckerRule extends UnionCheckerRule<ILongitudinalAction> {

  /** Default constructor */
  public LongitudinalActionUnionCheckerRule() {
    super();
  }
  ...
  private List<String> getNotNullChildren(ILongitudinalAction object) {
    List<String> propertyNamesNotNull = new ArrayList<>();
    if (object.getSpeedAction() != null) {
      propertyNamesNotNull.add(OscConstants.ELEMENT__SPEED_ACTION);
    }

    if (object.getLongitudinalDistanceAction() != null) {
      propertyNamesNotNull.add(OscConstants.ELEMENT__LONGITUDINAL_DISTANCE_ACTION);
    }
  ...
  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, ILongitudinalAction object) {
    List<String> propertyNamesNotNull = getNotNullChildren(object);
    if (propertyNamesNotNull.size() > 1) {
      messageLogger.logMessage(
