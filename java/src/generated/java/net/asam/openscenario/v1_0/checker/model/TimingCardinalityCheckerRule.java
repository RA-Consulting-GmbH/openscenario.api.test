 ...
 * cardinality checker rule class for the type ITiming according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class TimingCardinalityCheckerRule extends CardinalityCheckerRule<ITiming> {

  /** Default constructor */
  public TimingCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(ITiming object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getDomainAbsoluteRelative() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__DOMAIN_ABSOLUTE_RELATIVE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getScale() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__SCALE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
