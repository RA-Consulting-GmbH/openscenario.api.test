 ...
 * cardinality checker rule class for the type IPedestrian according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class PedestrianCardinalityCheckerRule extends CardinalityCheckerRule<IPedestrian> {

  /** Default constructor */
  public PedestrianCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IPedestrian object) {
  ...
  private List<CardinalityViolation> getAllViolations(IPedestrian object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getModel() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__MODEL, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getMass() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__MASS, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    ...
    // Check violation
    // Check required
    if (object.getPedestrianCategory() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__PEDESTRIAN_CATEGORY, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getBoundingBox() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__BOUNDING_BOX, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getProperties() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__PROPERTIES, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IPedestrian object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
