 ...
 * cardinality checker rule class for the type IAxle according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class AxleCardinalityCheckerRule extends CardinalityCheckerRule<IAxle> {

  /** Default constructor */
  public AxleCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IAxle object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getMaxSteering() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__MAX_STEERING, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getWheelDiameter() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__WHEEL_DIAMETER, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getTrackWidth() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__TRACK_WIDTH, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getPositionX() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__POSITION_X, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getPositionZ() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__POSITION_Z, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
