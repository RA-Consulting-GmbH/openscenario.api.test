 ...
 * cardinality checker rule class for the type IActors according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class ActorsCardinalityCheckerRule extends CardinalityCheckerRule<IActors> {

  /** Default constructor */
  public ActorsCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IActors object) {
  ...
  private List<CardinalityViolation> getAllViolations(IActors object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getSelectTriggeringEntities() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__SELECT_TRIGGERING_ENTITIES, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IActors object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
