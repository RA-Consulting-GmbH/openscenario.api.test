 ...
 * cardinality checker rule class for the type IScenarioDefinition according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class ScenarioDefinitionCardinalityCheckerRule
    extends CardinalityCheckerRule<IScenarioDefinition> {

  /** Default constructor */
  public ScenarioDefinitionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IScenarioDefinition object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getCatalogLocations() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__CATALOG_LOCATIONS, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getRoadNetwork() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__ROAD_NETWORK, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getEntities() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__ENTITIES, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getStoryboard() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__STORYBOARD, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IScenarioDefinition object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
