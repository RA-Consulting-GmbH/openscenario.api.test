 ...
 * cardinality checker rule class for the type ITrafficSignalState according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class TrafficSignalStateCardinalityCheckerRule
    extends CardinalityCheckerRule<ITrafficSignalState> {

  /** Default constructor */
  public TrafficSignalStateCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(ITrafficSignalState object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getTrafficSignalId() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__TRAFFIC_SIGNAL_ID, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
  ...
  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, ITrafficSignalState object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
