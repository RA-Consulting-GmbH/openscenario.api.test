 ...
 * cardinality checker rule class for the type IOverrideGearAction according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class OverrideGearActionCardinalityCheckerRule
    extends CardinalityCheckerRule<IOverrideGearAction> {

  /** Default constructor */
  public OverrideGearActionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IOverrideGearAction object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getNumber() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__NUMBER, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
  ...
  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IOverrideGearAction object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
