 ...
 * cardinality checker rule class for the type IOpenScenario according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class OpenScenarioCardinalityCheckerRule extends CardinalityCheckerRule<IOpenScenario> {

  /** Default constructor */
  public OpenScenarioCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IOpenScenario object) {
  ...
  private List<CardinalityViolation> getAllViolations(IOpenScenario object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getFileHeader() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__FILE_HEADER, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getOpenScenarioCategory() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__OPEN_SCENARIO_CATEGORY, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IOpenScenario object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
