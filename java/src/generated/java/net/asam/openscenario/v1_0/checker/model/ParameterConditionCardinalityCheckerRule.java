 ...
 * cardinality checker rule class for the type IParameterCondition according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class ParameterConditionCardinalityCheckerRule
    extends CardinalityCheckerRule<IParameterCondition> {

  /** Default constructor */
  public ParameterConditionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IParameterCondition object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getParameterRef() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__PARAMETER_REF, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
  ...
  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IParameterCondition object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
