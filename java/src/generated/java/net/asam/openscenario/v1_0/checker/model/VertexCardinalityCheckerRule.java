 ...
 * cardinality checker rule class for the type IVertex according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class VertexCardinalityCheckerRule extends CardinalityCheckerRule<IVertex> {

  /** Default constructor */
  public VertexCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IVertex object) {
  ...
  private List<CardinalityViolation> getAllViolations(IVertex object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getTime() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__TIME, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
  ...
  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IVertex object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
