 ...
 * cardinality checker rule class for the type IScenarioObject according to OpenSCENARIO standard
 * all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class ScenarioObjectCardinalityCheckerRule extends CardinalityCheckerRule<IScenarioObject> {

  /** Default constructor */
  public ScenarioObjectCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IScenarioObject object) {
  ...
  private List<CardinalityViolation> getAllViolations(IScenarioObject object) {
    ...
    // Check violation
    // Check required
    if (object.getEntityObject() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__ENTITY_OBJECT, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IScenarioObject object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
