 ...
 * cardinality checker rule class for the type IByEntityCondition according to OpenSCENARIO standard
 * all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class ByEntityConditionCardinalityCheckerRule
    extends CardinalityCheckerRule<IByEntityCondition> {

  /** Default constructor */
  public ByEntityConditionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IByEntityCondition object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getTriggeringEntities() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__TRIGGERING_ENTITIES, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getEntityCondition() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__ENTITY_CONDITION, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IByEntityCondition object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
