 ...
 * cardinality checker rule class for the type ITimeOfDay according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class TimeOfDayCardinalityCheckerRule extends CardinalityCheckerRule<ITimeOfDay> {

  /** Default constructor */
  public TimeOfDayCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, ITimeOfDay object) {
  ...
  private List<CardinalityViolation> getAllViolations(ITimeOfDay object) {
    ...
    if (object.getAnimation() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__ANIMATION, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getDateTime() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__DATE_TIME, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, ITimeOfDay object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
