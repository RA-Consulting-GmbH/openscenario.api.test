 ...
 * cardinality checker rule class for the type IRoutePosition according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class RoutePositionCardinalityCheckerRule extends CardinalityCheckerRule<IRoutePosition> {

  /** Default constructor */
  public RoutePositionCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IRoutePosition object) {
  ...
  private List<CardinalityViolation> getAllViolations(IRoutePosition object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getRouteRef() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__ROUTE_REF, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getInRoutePosition() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__IN_ROUTE_POSITION, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IRoutePosition object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
