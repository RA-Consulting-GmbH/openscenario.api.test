 ...
 * cardinality checker rule class for the type IRelativeSpeedToMaster according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class RelativeSpeedToMasterCardinalityCheckerRule
    extends CardinalityCheckerRule<IRelativeSpeedToMaster> {

  /** Default constructor */
  public RelativeSpeedToMasterCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IRelativeSpeedToMaster object) {
    ...
    // Check violation
    // Check required
    if (object.getSpeedTargetValueType() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__SPEED_TARGET_VALUE_TYPE, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, IRelativeSpeedToMaster object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
