 ...
 * cardinality checker rule class for the type IFog according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class FogCardinalityCheckerRule extends CardinalityCheckerRule<IFog> {

  /** Default constructor */
  public FogCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IFog object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getVisualRange() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__VISUAL_RANGE, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
