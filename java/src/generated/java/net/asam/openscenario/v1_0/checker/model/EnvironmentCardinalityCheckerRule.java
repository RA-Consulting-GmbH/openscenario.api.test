 ...
 * cardinality checker rule class for the type IEnvironment according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class EnvironmentCardinalityCheckerRule extends CardinalityCheckerRule<IEnvironment> {

  /** Default constructor */
  public EnvironmentCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IEnvironment object) {
  ...
  private List<CardinalityViolation> getAllViolations(IEnvironment object) {
    ...
    // Check violation
    // Check required
    if (object.getTimeOfDay() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__TIME_OF_DAY, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getWeather() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__WEATHER, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getRoadCondition() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__ROAD_CONDITION, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IEnvironment object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
