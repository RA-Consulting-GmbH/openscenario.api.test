 ...
 * cardinality checker rule class for the type IDistanceCondition according to OpenSCENARIO standard
 * all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class DistanceConditionCardinalityCheckerRule
    extends CardinalityCheckerRule<IDistanceCondition> {

  /** Default constructor */
  public DistanceConditionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IDistanceCondition object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getValue() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__VALUE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getFreespace() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__FREESPACE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getAlongRoute() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__ALONG_ROUTE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getRule() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__RULE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getPosition() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__POSITION, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IDistanceCondition object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
