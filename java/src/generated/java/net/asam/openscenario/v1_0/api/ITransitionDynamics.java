 ...
 *
 * <p>From OpenSCENARIO class model specification: Specifies the dynamics of a value transition and
 * defines how the value changes over time or distance. Different shapes can be used for the
 * transition. Step is an immediate transition, i.e. a jump from the current current value to the
 * target value. In this case value for time or distance must be 0. Linear will result in a linear
 * transition between the start and end value. A smooth transition is only given with cubic and
 * sinusoidal shape.
 *
