 ...
 * cardinality checker rule class for the type IFile according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class FileCardinalityCheckerRule extends CardinalityCheckerRule<IFile> {

  /** Default constructor */
  public FileCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IFile object) {
  ...
  private List<CardinalityViolation> getAllViolations(IFile object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getFilepath() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__FILEPATH, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IFile object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
