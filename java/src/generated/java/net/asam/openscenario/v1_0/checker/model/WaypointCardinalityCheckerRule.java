 ...
 * cardinality checker rule class for the type IWaypoint according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class WaypointCardinalityCheckerRule extends CardinalityCheckerRule<IWaypoint> {

  /** Default constructor */
  public WaypointCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IWaypoint object) {
  ...
  private List<CardinalityViolation> getAllViolations(IWaypoint object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getRouteStrategy() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__ROUTE_STRATEGY, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
  ...
  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IWaypoint object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
