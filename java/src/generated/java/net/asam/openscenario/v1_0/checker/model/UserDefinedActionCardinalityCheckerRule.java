 ...
 * cardinality checker rule class for the type IUserDefinedAction according to OpenSCENARIO standard
 * all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class UserDefinedActionCardinalityCheckerRule
    extends CardinalityCheckerRule<IUserDefinedAction> {

  /** Default constructor */
  public UserDefinedActionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IUserDefinedAction object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getCustomCommandAction() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__CUSTOM_COMMAND_ACTION, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IUserDefinedAction object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
