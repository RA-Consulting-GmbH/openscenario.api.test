 ...
 * cardinality checker rule class for the type ITrajectoryFollowingMode according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class TrajectoryFollowingModeCardinalityCheckerRule
    extends CardinalityCheckerRule<ITrajectoryFollowingMode> {

  /** Default constructor */
  public TrajectoryFollowingModeCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(ITrajectoryFollowingMode object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getFollowingMode() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__FOLLOWING_MODE, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, ITrajectoryFollowingMode object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
