 ...
 * cardinality checker rule class for the type IParameterModifyAction according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class ParameterModifyActionCardinalityCheckerRule
    extends CardinalityCheckerRule<IParameterModifyAction> {

  /** Default constructor */
  public ParameterModifyActionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IParameterModifyAction object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getRule() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__RULE, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, IParameterModifyAction object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
