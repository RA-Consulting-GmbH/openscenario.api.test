 ...
 * cardinality checker rule class for the type ICondition according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class ConditionCardinalityCheckerRule extends CardinalityCheckerRule<ICondition> {

  /** Default constructor */
  public ConditionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(ICondition object) {
    ...
    if (object.getDelay() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__DELAY, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getConditionEdge() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__CONDITION_EDGE, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, ICondition object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
