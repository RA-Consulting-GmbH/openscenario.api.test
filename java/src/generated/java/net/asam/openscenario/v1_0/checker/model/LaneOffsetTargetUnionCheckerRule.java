 ...
 * union checker rule class for the type ILaneOffsetTarget according to OpenSCENARIO standard all
 * xor elements are checked
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class LaneOffsetTargetUnionCheckerRule extends UnionCheckerRule<ILaneOffsetTarget> {

  /** Default constructor */
  public LaneOffsetTargetUnionCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, ILaneOffsetTarget object) {
  ...
  private List<String> getNotNullChildren(ILaneOffsetTarget object) {
    List<String> propertyNamesNotNull = new ArrayList<>();
    if (object.getRelativeTargetLaneOffset() != null) {
      propertyNamesNotNull.add(OscConstants.ELEMENT__RELATIVE_TARGET_LANE_OFFSET);
    }

    if (object.getAbsoluteTargetLaneOffset() != null) {
      propertyNamesNotNull.add(OscConstants.ELEMENT__ABSOLUTE_TARGET_LANE_OFFSET);
    }
  ...
  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, ILaneOffsetTarget object) {
    List<String> propertyNamesNotNull = getNotNullChildren(object);
    if (propertyNamesNotNull.size() > 1) {
      messageLogger.logMessage(
