 ...
 * cardinality checker rule class for the type IVehicle according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class VehicleCardinalityCheckerRule extends CardinalityCheckerRule<IVehicle> {

  /** Default constructor */
  public VehicleCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IVehicle object) {
    ...
    // Check violation
    // Check required
    if (object.getVehicleCategory() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__VEHICLE_CATEGORY, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    ...
    if (object.getPerformance() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ELEMENT__PERFORMANCE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getAxles() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__AXLES, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
