 ...
 * cardinality checker rule class for the type ITrajectory according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class TrajectoryCardinalityCheckerRule extends CardinalityCheckerRule<ITrajectory> {

  /** Default constructor */
  public TrajectoryCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, ITrajectory object) {
  ...
  private List<CardinalityViolation> getAllViolations(ITrajectory object) {
    ...
    if (object.getShape() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__SHAPE, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, ITrajectory object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
