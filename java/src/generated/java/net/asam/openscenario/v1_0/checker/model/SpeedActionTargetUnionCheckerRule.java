 ...
 * union checker rule class for the type ISpeedActionTarget according to OpenSCENARIO standard all
 * xor elements are checked
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class SpeedActionTargetUnionCheckerRule extends UnionCheckerRule<ISpeedActionTarget> {

  /** Default constructor */
  public SpeedActionTargetUnionCheckerRule() {
    super();
  }
  ...
  private List<String> getNotNullChildren(ISpeedActionTarget object) {
    List<String> propertyNamesNotNull = new ArrayList<>();
    if (object.getRelativeTargetSpeed() != null) {
      propertyNamesNotNull.add(OscConstants.ELEMENT__RELATIVE_TARGET_SPEED);
    }

    if (object.getAbsoluteTargetSpeed() != null) {
      propertyNamesNotNull.add(OscConstants.ELEMENT__ABSOLUTE_TARGET_SPEED);
    }
  ...
  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, ISpeedActionTarget object) {
    List<String> propertyNamesNotNull = getNotNullChildren(object);
    if (propertyNamesNotNull.size() > 1) {
      messageLogger.logMessage(
