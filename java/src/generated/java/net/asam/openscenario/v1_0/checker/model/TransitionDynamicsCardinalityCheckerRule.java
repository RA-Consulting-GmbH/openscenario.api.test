 ...
 * cardinality checker rule class for the type ITransitionDynamics according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class TransitionDynamicsCardinalityCheckerRule
    extends CardinalityCheckerRule<ITransitionDynamics> {

  /** Default constructor */
  public TransitionDynamicsCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(ITransitionDynamics object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getDynamicsShape() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__DYNAMICS_SHAPE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    ...
    // Check violation
    // Check required
    if (object.getDynamicsDimension() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__DYNAMICS_DIMENSION, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, ITransitionDynamics object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
