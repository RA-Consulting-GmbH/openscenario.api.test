 ...
 * cardinality checker rule class for the type IEndOfRoadCondition according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class EndOfRoadConditionCardinalityCheckerRule
    extends CardinalityCheckerRule<IEndOfRoadCondition> {

  /** Default constructor */
  public EndOfRoadConditionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IEndOfRoadCondition object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getDuration() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__DURATION, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IEndOfRoadCondition object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
