 ...
 *
 * <p>From OpenSCENARIO class model specification: This action describes the transition between an
 * entity's current lane and its target lane. The target lane may be given in absolute or relative
 * terms. The dynamics of the transition may be given either by providing the time or the distance
 * required for performing the transition. Time and distance are measured between the start position
 * and the end position of the action.. The transition starts at the current lane position,
 * including the current lane offset and ends at the target lane position, optionally including a
 * targetLaneOffset.
 *
