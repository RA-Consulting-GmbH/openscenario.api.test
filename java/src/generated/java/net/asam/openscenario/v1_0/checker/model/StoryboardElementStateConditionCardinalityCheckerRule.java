 ...
 * cardinality checker rule class for the type IStoryboardElementStateCondition according to
 * OpenSCENARIO standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class StoryboardElementStateConditionCardinalityCheckerRule
    extends CardinalityCheckerRule<IStoryboardElementStateCondition> {

  /** Default constructor */
  public StoryboardElementStateConditionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IStoryboardElementStateCondition object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getStoryboardElementType() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__STORYBOARD_ELEMENT_TYPE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getStoryboardElementRef() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__STORYBOARD_ELEMENT_REF, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getState() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__STATE, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, IStoryboardElementStateCondition object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
