 ...
 * cardinality checker rule class for the type IParameterDeclaration according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class ParameterDeclarationCardinalityCheckerRule
    extends CardinalityCheckerRule<IParameterDeclaration> {

  /** Default constructor */
  public ParameterDeclarationCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IParameterDeclaration object) {
    ...
    if (object.getParameterType() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__PARAMETER_TYPE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, IParameterDeclaration object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
