 ...
 * cardinality checker rule class for the type IRoadPosition according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class RoadPositionCardinalityCheckerRule extends CardinalityCheckerRule<IRoadPosition> {

  /** Default constructor */
  public RoadPositionCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IRoadPosition object) {
  ...
  private List<CardinalityViolation> getAllViolations(IRoadPosition object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getRoadId() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__ROAD_ID, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getS() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__S, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getT() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__T, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IRoadPosition object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
