 ...
 * cardinality checker rule class for the type IMiscObject according to OpenSCENARIO standard all
 * properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class MiscObjectCardinalityCheckerRule extends CardinalityCheckerRule<IMiscObject> {

  /** Default constructor */
  public MiscObjectCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IMiscObject object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getMiscObjectCategory() == null) {
      violations.add(
          new CardinalityViolation(
              OscConstants.ATTRIBUTE__MISC_OBJECT_CATEGORY, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
