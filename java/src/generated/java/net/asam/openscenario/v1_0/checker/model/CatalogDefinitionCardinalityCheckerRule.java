 ...
 * cardinality checker rule class for the type ICatalogDefinition according to OpenSCENARIO standard
 * all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class CatalogDefinitionCardinalityCheckerRule
    extends CardinalityCheckerRule<ICatalogDefinition> {

  /** Default constructor */
  public CatalogDefinitionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(ICatalogDefinition object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getCatalog() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__CATALOG, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, ICatalogDefinition object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
