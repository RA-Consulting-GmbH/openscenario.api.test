 ...
 * cardinality checker rule class for the type ITriggeringEntities according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class TriggeringEntitiesCardinalityCheckerRule
    extends CardinalityCheckerRule<ITriggeringEntities> {

  /** Default constructor */
  public TriggeringEntitiesCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(ITriggeringEntities object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getTriggeringEntitiesRule() == null) {
      violations.add(
  ...
  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, ITriggeringEntities object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
