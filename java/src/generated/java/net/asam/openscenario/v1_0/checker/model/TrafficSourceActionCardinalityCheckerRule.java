 ...
 * cardinality checker rule class for the type ITrafficSourceAction according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class TrafficSourceActionCardinalityCheckerRule
    extends CardinalityCheckerRule<ITrafficSourceAction> {

  /** Default constructor */
  public TrafficSourceActionCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(ITrafficSourceAction object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getRate() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__RATE, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
    if (object.getRadius() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ATTRIBUTE__RADIUS, 0, 1, ViolationType.REQUIRED));
    }
    // Check violation
    // Check required
