 ...
 * cardinality checker rule class for the type IEnvironmentCatalogLocation according to OpenSCENARIO
 * standard all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class EnvironmentCatalogLocationCardinalityCheckerRule
    extends CardinalityCheckerRule<IEnvironmentCatalogLocation> {

  /** Default constructor */
  public EnvironmentCatalogLocationCardinalityCheckerRule() {
    super();
  }
  ...
  private List<CardinalityViolation> getAllViolations(IEnvironmentCatalogLocation object) {
    List<CardinalityViolation> violations = new ArrayList<>();
    // Check violation
    // Check required
    if (object.getDirectory() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__DIRECTORY, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }
  ...
  public void applyRuleInTreeContext(
      ITreeMessageLogger messageLogger, IEnvironmentCatalogLocation object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
