 ...
 * cardinality checker rule class for the type IEntitySelection according to OpenSCENARIO standard
 * all properties and children are checked if they are not optinal
 *
 * @author RA Consulting OpenSCENARIO generation facility
 */
public class EntitySelectionCardinalityCheckerRule
    extends CardinalityCheckerRule<IEntitySelection> {

  /** Default constructor */
  public EntitySelectionCardinalityCheckerRule() {
    super();
  }

  @Override
  public void applyRuleInFileContext(IParserMessageLogger messageLogger, IEntitySelection object) {
  ...
  private List<CardinalityViolation> getAllViolations(IEntitySelection object) {
    ...
    if (object.getMembers() == null) {
      violations.add(
          new CardinalityViolation(OscConstants.ELEMENT__MEMBERS, 0, 1, ViolationType.REQUIRED));
    }
    return violations;
  }

  @Override
  public void applyRuleInTreeContext(ITreeMessageLogger messageLogger, IEntitySelection object) {
    List<CardinalityViolation> violations = getAllViolations(object);
    for (CardinalityViolation violation : violations) {
      String message = getMessage(violation);
