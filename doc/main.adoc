...
[source,bash]
----
$ ./generateWindows.sh (all (VS2010|...|VS2022) [release] [shared|static] [Win32|x64]) |
((VS2010|...|VS2022) [release|debug] [shared|static] [Win32|x64] [make])
----
Option ```all [VS2019] [release]``` builds project files for Visual Studio 2019 and compiles static and shared as well as release and optional debug versions of OpenSCENARIO in parallel.
On the other hand you can build and optionally compile individual versions of OpenSCENARIO, e.g.:
[source,bash]
----
> generateWindows.bat VS2019 x64 shared release
...
===== Via Visual Studio
Here we have a already preselected the startup project "OpenScenarioTester".
...
Just change directory to that folder and start the test:
