...
Please see link:{levelToBaseDir}/cpp/applications/openScenarioTester/src/TestInjectedParameters.h[TestInjectedParameters.h] for detailed tests on error handling.

== General architectural aspects
This article is about some fundamental aspects in the API's architecture. It may help to understand the intention behind the design decisions and also gives you a entry point in coding and integration
issues.
...
If your CMake version is older than 3.19 then cmake-gui is missing the "Preset" entry as seen in the two images below.
...
You still can use older versions of cmake-gui but in order to work properly how have to select in advance the preset manually using the command line.

For example the command for Linux would look like that:
[source,bash]
----
$ cmake --preset="Linux-shared-release"
----
And for Windows the command would look like that:
[source,bash]
----
> cmake --preset="VS2019-x64-shared"
----

After entering one of these commands you can switch back to cmake-gui.

See section <<BuildingOnConsoleTerminal,Building on console / terminal>> for further building instructionsfor command line.
A complete command sequence for building OpenSCENARIO via command line is given in section <<CommandLineStepsCondensed,Command line steps condensed>>.
