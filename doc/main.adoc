...
* <<BuildingWithCmakeVersionLe3.19,Building with cmake version >= 3.19>>
...
(work in progress)
//The Windows version of the script is almost identical to the Linux version.
//It has only one additional parameter to choose between a 32 Bit or 64 Bit version.
//
//On Windows the commands would look like that:
//[source,bash]
//----
//> cd openscenario.api.test\cpp\build
//> generateWindows.bat shared release make
// ----
